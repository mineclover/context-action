...
function UseActionWithResultExample() {
  ...
  const handleCompleteWorkflow = async () => {
    ...
    try {
      ...
      // 3단계: 주문 처리
      const processResult = await dispatchWithResult(
        'processOrder',
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'monospace' }}>
      ...
      >
        <h3>현재 상태</h3>
        <div>
          <strong>Cart Items:</strong> {cartItems.length}개
        </div>
        <div>
        ...
        <div>
          ...
          {calculation ? `$${calculation.total}` : '미계산'}
        </div>
        <div>
          <strong>Order:</strong> {order ? order.status : '미처리'}
        </div>
      </div>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
          ...
          <button onClick={handleParallelExecution}>4. 병렬 실행</button>
          <button onClick={handleMergedResults}>5. 결과 병합</button>
        </div>
      </div>
      ...
      >
        ...
        <ul>
          <li>
            <strong>결과 수집:</strong> 핸들러 실행 결과를 자동 수집
          </li>
          <li>
            <strong>실행 정보:</strong> 실행 시간, 성공/실패, 에러 정보 제공
          </li>
          <li>
            <strong>필터링:</strong> 태그, 카테고리, 환경별 핸들러 필터링
          </li>
          <li>
            <strong>결과 전략:</strong> first, last, all, merge, custom 전략
            지원
          </li>
          <li>
            <strong>실행 모드:</strong> sequential, parallel, race 모드 지원
          </li>
          <li>
