...
export function ConcurrentActionTestPage() {
  ...
  const createOrUpdateTask = (taskId: string, name: string, updates: Partial<TaskStatus> = {}) => {
    setTasks(prev => {
  ...
  useEffect(() => {
    ...
    unregisterFunctions.push(
      register.register('longRunningTaskA', async ({ taskId, duration }, controller) => {
        const taskName = `Task A (${duration}ms)`;
        addLog('longRunningTaskA', 'started', `Task ID: ${taskId}, Duration: ${duration}ms`);
        ...
        try {
          ...
          for (let i = 0; i <= steps; i++) {
            ...
            try {
              ...
              addLog('longRunningTaskA', 'aborted', `Task ${taskId} aborted at step ${i}/${steps}`);
              updateTask(taskId, { 
                status: 'aborted', 
                endTime: Date.now(),
          ...
          addLog('longRunningTaskA', 'completed', `Task ${taskId} completed successfully`);
          updateTask(taskId, { 
            status: 'completed', 
            endTime: Date.now(),
            progress: 100,
          ...
          addLog('longRunningTaskA', 'failed', `Task ${taskId} failed: ${error}`);
          updateTask(taskId, { 
            status: 'failed', 
            endTime: Date.now(),
            error: String(error) 
    ...
    unregisterFunctions.push(
      register.register('longRunningTaskB', async ({ taskId, duration }, controller) => {
        const taskName = `Task B (${duration}ms)`;
        addLog('longRunningTaskB', 'started', `Task ID: ${taskId}, Duration: ${duration}ms`);
        ...
        try {
          ...
          for (let i = 0; i <= steps; i++) {
            try {
              ...
              addLog('longRunningTaskB', 'aborted', `Task ${taskId} aborted at step ${i}/${steps}`);
              updateTask(taskId, { 
                status: 'aborted', 
                endTime: Date.now(),
                progress: (i / steps) * 100 
            ...
            if (i === Math.floor(steps / 2)) {
              addLog('longRunningTaskB', 'pipeline-interaction', 
              ...
              try {
                ...
                addLog('longRunningTaskB', 'pipeline-interaction-failed', 
                  `Secondary dispatch failed: ${err}`);
              }
            }
          }

          addLog('longRunningTaskB', 'completed', `Task ${taskId} completed successfully`);
          updateTask(taskId, { 
            status: 'completed', 
            endTime: Date.now(),
            progress: 100,
          ...
          addLog('longRunningTaskB', 'failed', `Task ${taskId} failed: ${error}`);
          updateTask(taskId, { 
            status: 'failed', 
            endTime: Date.now(),
            error: String(error) 
    ...
    unregisterFunctions.push(
      register.register('longRunningTaskC', async ({ taskId, duration }, controller) => {
        const taskName = `Task C (${duration}ms)`;
        addLog('longRunningTaskC', 'started', `Task ID: ${taskId}, Duration: ${duration}ms`);
        ...
        try {
          ...
          for (let i = 0; i <= steps; i++) {
            try {
              ...
              addLog('longRunningTaskC', 'aborted', `Task ${taskId} aborted at step ${i}/${steps}`);
              updateTask(taskId, { 
                status: 'aborted', 
                endTime: Date.now(),
                progress: (i / steps) * 100 
          ...
          addLog('longRunningTaskC', 'completed', `Task ${taskId} completed successfully`);
          updateTask(taskId, { 
            status: 'completed', 
            endTime: Date.now(),
            progress: 100,
          ...
          addLog('longRunningTaskC', 'failed', `Task ${taskId} failed: ${error}`);
          updateTask(taskId, { 
            status: 'failed', 
            endTime: Date.now(),
            error: String(error) 
    ...
    unregisterFunctions.push(
      register.register('apiCallSecondary', async ({ endpoint, params }) => {
        addLog('apiCallSecondary', 'started', `Calling ${endpoint} with params: ${JSON.stringify(params)}`);
        
        // Simulate API call
        await new Promise(resolve => setTimeout(resolve, 100));
        
        addLog('apiCallSecondary', 'completed', `API call to ${endpoint} completed`);
      })
    );

    // Background job handler
    unregisterFunctions.push(
      register.register('backgroundJob', async ({ jobId, jobType, priority }, controller) => {
        const taskName = `Background Job (${jobType}, Priority: ${priority})`;
        addLog('backgroundJob', 'started', `Job ID: ${jobId}, Type: ${jobType}, Priority: ${priority}`);
        ...
        try {
          ...
          for (let i = 0; i <= steps; i++) {
            try {
              ...
              addLog('backgroundJob', 'aborted', `Job ${jobId} aborted at step ${i}/${steps}`);
              updateTask(jobId, { 
                status: 'aborted', 
                endTime: Date.now(),
                progress: (i / steps) * 100 
  ...
  const runSingleTask = async (taskType: 'A' | 'B' | 'C', duration: number) => {
    ...
    addLog(actionMap[taskType], 'dispatched', `Dispatching ${taskType} with duration ${duration}ms`);
    
    try {
      await dispatch(actionMap[taskType], { taskId, duration });
    } catch (error) {
      addLog(actionMap[taskType], 'dispatch-error', `Failed to dispatch: ${error}`);
    }
  };

  const runMultipleTasks = async () => {
    const timestamp = Date.now();
    addLog('multiple-tasks', 'started', 'Starting concurrent execution of tasks A, B, and C');
    ...
    const promises = [
      dispatch('longRunningTaskA', { taskId: `concurrent-a-${timestamp}`, duration: 2000 }),
      dispatch('longRunningTaskB', { taskId: `concurrent-b-${timestamp}`, duration: 3000 }),
      dispatch('longRunningTaskC', { taskId: `concurrent-c-${timestamp}`, duration: 1500 })
    ];
  ...
  const abortAllTasks = () => {
    ...
    addLog('abort', 'triggered', `Aborting ${runningTasks.length} running tasks`);
  ...
  const resetAbortScopeHandler = () => {
    addLog('abort-scope', 'reset', 'Creating new abort scope for future actions');
    resetAbortScope();
  };
  ...
  return (
    <div className="concurrent-action-test-page">
      <div className="max-w-7xl mx-auto p-6 space-y-8">
        <header>
          <h1 className="text-3xl font-bold text-gray-900">Concurrent Action Test</h1>
          <p className="mt-2 text-gray-600">
            Test simultaneous action execution, abort functionality, and pipeline interactions
          </p>
        </header>
        ...
        <Card title="Control Panel">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <Button 
              onClick={() => runSingleTask('A', 2000)}
              variant="primary"
            >
              Run Task A (2s)
            </Button>
            <Button 
              onClick={() => runSingleTask('B', 3000)}
              variant="primary"
            >
              Run Task B (3s)
            </Button>
            <Button 
              onClick={() => runSingleTask('C', 1500)}
              variant="primary"
            >
              Run Task C (1.5s)
            </Button>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <Button 
              onClick={runPipelineInteractionTest}
              variant="secondary"
            >
            ...
            <Button 
              onClick={resetAbortScopeHandler}
              variant="warning"
            >
              Reset Abort Scope
            </Button>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm">
            <div className="text-center">
              <div className="font-semibold text-blue-600">{runningTasks.length}</div>
            ...
            <div className="text-center">
              <div className="font-semibold text-yellow-600">{abortedTasks.length}</div>
              <div className="text-gray-500">Aborted</div>
            </div>
            <div className="text-center">
              <div className="font-semibold text-red-600">{failedTasks.length}</div>
              <div className="text-gray-500">Failed</div>
            </div>
            <div className="text-center">
              <div className="font-semibold text-purple-600">{globalAbortCount}</div>
                ...
                tasks.slice(-10).reverse().map(task => (
                  <div key={task.id} className="border rounded-lg p-3 bg-gray-50">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Status status={
                        task.status === 'completed' ? 'safe' :
                        task.status === 'running' ? 'warning' :
                        task.status === 'aborted' ? 'danger' :
                    ...
                    {task.result && (
                      <p className="text-xs text-green-600 mt-2">{task.result}</p>
                    )}
          ...
          <Card title="Execution Logs">
            <div className="space-y-1 max-h-96 overflow-y-auto font-mono text-xs">
              {executionLogs.length === 0 ? (
                ...
                executionLogs.slice(-20).reverse().map((log, index) => (
                  <div key={`${log.timestamp}-${index}`} className="border-l-2 border-gray-200 pl-3 py-1">
                    <div className="flex justify-between items-start">
                      <span className={`font-medium ${
                        log.event.includes('completed') ? 'text-green-600' :
                        log.event.includes('aborted') ? 'text-yellow-600' :
                        log.event.includes('failed') || log.event.includes('error') ? 'text-red-600' :
                        log.event.includes('started') ? 'text-blue-600' : 'text-gray-600'
                      }`}>
                        {log.action}
                      </span>
        ...
        <Card title="Test Information">
          <div className="prose prose-sm max-w-none">
            ...
            <ul>
              <li><strong>Concurrent Execution:</strong> Multiple actions with different names running simultaneously</li>
              <li><strong>Abort Functionality:</strong> Individual and bulk abortion of running actions</li>
              <li><strong>Pipeline Interactions:</strong> Actions dispatching other actions during execution</li>
              <li><strong>Progress Tracking:</strong> Real-time progress updates and status monitoring</li>
              <li><strong>Error Handling:</strong> Graceful handling of failures and aborts</li>
              <li><strong>Scope Management:</strong> resetAbortScope() creates new abort contexts</li>
            </ul>
            
            <h4>Key Insights:</h4>
            <ul>
              <li>Actions with different names can run concurrently without interference</li>
              ...
              <li>Actions can dispatch other actions during their execution (Task B → API Secondary)</li>
              <li>Individual action progress and lifecycle can be tracked independently</li>
            </ul>
          </div>
        </Card>
      </div>
