...
export function useLogMonitorContext(): LogMonitorContextValue {
  ...
  if (!context) {
    throw new Error('useLogMonitorContext must be used within a LogMonitorProvider');
  }
  return context;
}
...
export function LogMonitorProvider({ 
  ...
  const stores: LogMonitorStores = useMemo(() => {
    return logMonitorStoreRegistry.getStores(pageId, initialLogLevel, initialConfig);
  ...
  const fallbackConfig = useMemo(() => ({ 
    ...
    ...initialConfig 
  } as LogMonitorConfig), [initialLogLevel, initialConfig]);
...
interface PageWithLogMonitorProps {
  ...
  pageId: string;
  title?: string;
  logMonitorProps?: Record<string, any>;
  initialLogLevel?: LogLevel;
  initialConfig?: Partial<LogMonitorConfig>;
}
...
export function PageWithLogMonitor({ 
  ...
  // 동적 import를 통해 LogMonitor 컴포넌트 로드
  const [LogMonitor, setLogMonitor] = React.useState<React.ComponentType<any> | null>(null);

  useEffect(() => {
    import('./LogMonitor').then(module => {
      setLogMonitor(() => module.LogMonitor);
    });
  }, []);
