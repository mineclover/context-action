...
const _FUN_FACTS = [
  'loves coffee and debugging',
  'speaks 5 programming languages',
  'builds robots in spare time',
  'writes poetry about algorithms',
  'collects vintage keyboards',
  'dreams in binary',
  "can solve a Rubik's cube in 30 seconds",
  'has read all of Stack Overflow',
  'once found a bug by staring at code',
  'types 120 WPM',
  'prefers vim over emacs',
  'thinks semicolons are optional',
  'believes in 10x engineers',
  'codes while sleeping',
];

function generateFakeUserData(): UserFormData {
  const name =
    FAKE_NAMES[Math.floor(Math.random() * FAKE_NAMES.length)] || 'Unknown User';
  const emailPrefix =
    name
      .toLowerCase()
      .replace(/\s+/g, '.')
  ...
  const role =
    FAKE_ROLES[Math.floor(Math.random() * FAKE_ROLES.length)] || 'user';
...
export const UserFormView = memo<UserFormViewProps>(
  ({
    ...
    const getFieldError = (
      fieldName: keyof Pick<UserFormData, 'name' | 'email'>
    ) => {
    ...
    const handleFillFakeData = async () => {
      ...
      // Fun console message
      console.log(
        `🎉 Generated fake user: ${fakeData.name} (${fakeData.email}) - ${fakeData.role}`
      );
    };
