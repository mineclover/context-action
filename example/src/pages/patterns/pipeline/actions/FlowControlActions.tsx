...
import type { ApiResult, ScenarioKey } from '../scenarios/types';
...
export function FlowControlActions({ children }: FlowControlActionsProps) {
  ...
  const adjustSystemLoad = useCallback(
    (load: number) => {
      const currentState = demoStateStore.getValue();
  ...
  const onExecutionStep = useCallback(
    (step: string) => {
      const currentState = demoStateStore.getValue();
...
const FlowControlActionsContext =
  React.createContext<FlowControlActionsContextType | null>(null);

export function useFlowControlActions() {
  const context = React.useContext(FlowControlActionsContext);
  if (!context) {
    throw new Error(
      'useFlowControlActions must be used within FlowControlActions'
    );
  }
  return context;
}
