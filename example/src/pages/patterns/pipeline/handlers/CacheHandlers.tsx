...
export function CacheHandlers({ onExecutionStep, onHandlerExecution }: CacheHandlerProps) {
  ...
  const memoryCacheHandler = useCallback(async (payload: any, controller: any): Promise<void> => {
  ...
  const redisCacheHandler = useCallback(async (payload: any, controller: any): Promise<void> => {
  ...
  const databaseHandler = useCallback(async (payload: any, controller: any): Promise<void> => {
    onExecutionStep('🗄️ Database Fetch (P:60)');
    onHandlerExecution();
    
    console.log('🗄️ Fetching from database:', payload.fallbackUrl || payload.key);
  ...
  useCacheActionHandler('fetchData', memoryCacheHandler, { priority: 100, blocking: true });
  useCacheActionHandler('fetchData', redisCacheHandler, { priority: 80, blocking: true });
  useCacheActionHandler('fetchData', databaseHandler, { priority: 60, blocking: true });
