...
function TestHandlers({ children }: { children: React.ReactNode }) {
  ...
  // 렌더 카운트 증가 핸들러
  useTestActionHandler('incrementRenderCount', useCallback(async (payload) => {
  ...
  // 테스트 토글 핸들러
  useTestActionHandler('toggleTest', useCallback(async (payload) => {
  ...
  // 렌더 카운트 리셋 핸들러
  useTestActionHandler('resetRenderCounts', useCallback(async () => {
    renderCountsStore.setValue({});
  }, [renderCountsStore]));
...
function useTestActions() {
  ...
  return {
    incrementRenderCount: useCallback((componentId: string) => {
      dispatch('incrementRenderCount', { componentId });
    }, [dispatch]),
    
    toggleTest: useCallback((testId: string) => {
...
function RefStateMonitor({ 
  ...
  }, [isMounted, isWaitingForMount, mountedTarget, incrementRenderCount, refName]);
  ...
  useEffect(() => {
    ...
    if (prevMountedRef.current !== undefined && prevMountedRef.current !== isMounted) {
      logMonitor.addLog({
    ...
    <div className="p-3 bg-white rounded border">
      ...
      <div className="space-y-1 text-xs">
        <div className="flex justify-between">
          <span>isMounted:</span>
          <span className={isMounted ? 'text-green-600 font-bold' : 'text-red-600'}>
            {isMounted ? '✅' : '❌'}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className={isWaitingForMount ? 'text-yellow-600 font-bold' : 'text-gray-400'}>
            {isWaitingForMount ? '⏳' : '⏹️'}
          </span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className={mountedTarget ? 'text-blue-600 font-bold' : 'text-gray-400'}>
            {mountedTarget ? '📍' : '∅'}
          </span>
        </div>
...
function DynamicMountTest() {
  ...
  const toggleMount = useCallback(() => {
    ...
    console.log(`🔄 동적 요소 토글: ${isMounted ? 'mounted' : 'unmounted'} → ${newState ? 'mounted' : 'unmounted'}`);
  }, [isMounted]);

  // 마운트 상태 변경 콜백 (중복 호출 방지를 위한 최적화)
  const lastMountStateRef = useRef<{ mounted: boolean; timestamp: number } | null>(null);
  
  useTestOnMountStateChange('dynamicElement', useCallback((mounted, target) => {
    const now = Date.now();
    
    // 중복 호출 방지: 같은 상태가 100ms 이내에 반복 호출되면 무시
    if (lastMountStateRef.current && 
    ...
    if (mounted && target) {
      ...
      // 콘솔 로그로만 기록
      console.log('✅ 동적 요소 마운트됨 - useOnMountStateChange Hook', { target, timestamp: now });
    } else {
...
function ConditionalRenderTest() {
  ...
  const [condition, setCondition] = useState<'hidden' | 'loading' | 'shown'>('hidden');
  const actionLogger = useActionLoggerWithToast();
  const logMonitor = useLogMonitor();
  ...
  const startConditionalFlow = useCallback(() => {
    ...
    // 조건부 렌더링 시작 로깅
    actionLogger.logAction('startConditionalFlow',
    ...
    setTimeout(() => {
      ...
      // 성공 로깅 (Toast + 성능 데이터)
      actionLogger.logAction('conditionalElementShown',
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div className="text-sm text-gray-600">
          상태: {condition === 'hidden' ? '숨김' : condition === 'loading' ? '로딩 중' : '표시됨'}
        </div>
      </div>
...
function DelayedMountTest() {
  ...
  const startDelayedMount = useCallback(() => {
    ...
    // 3초 지연 마운트 시작 로깅
    actionLogger.logAction('startDelayedMount',
    ...
    const timer = setTimeout(() => {
      ...
      // 지연 마운트 완료 로깅
      actionLogger.logAction('delayedMountCompleted',
  ...
  const cancelDelayedMount = useCallback(() => {
    if (delayTimer) {
      ...
      // 취소 액션 로깅 (Toast 지원)
      actionLogger.logAction('cancelDelayedMount',
  ...
  const checkCurrentState = useCallback(() => {
    ...
    // 상태 확인 액션 로깅
    actionLogger.logAction('checkDelayedElementState',
      { 
        state: {
          isMounted: currentState.isMounted,
          isWaitingForMount: currentState.isWaitingForMount,
    ...
    <div className="space-y-4">
      ...
      {isDelayMounting && (
        <div className="p-4 border-2 border-dashed border-purple-300 rounded text-center bg-purple-50">
          ⏰ 3초 후에 요소가 마운트됩니다... ({Math.ceil((delayTimer ? 3 : 0))}초 남음)
        </div>
      )}
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto p-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold mb-4">🎯 useRefMountState Context-Layered Test</h1>
      ...
      <div className="bg-yellow-50 p-6 rounded-lg border border-yellow-200">
        <h3 className="text-lg font-bold mb-3">🏗️ Context-Layered Architecture</h3>
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <h4 className="font-semibold text-green-700">✅ 레이어 구조</h4>
            <ul className="text-sm space-y-1 mt-2 text-green-600">
              <li>• <strong>contexts/:</strong> TestStoreProvider, TestActionProvider, TestRefsProvider</li>
              <li>• <strong>handlers/:</strong> TestHandlers (비즈니스 로직)</li>
              <li>• <strong>actions/:</strong> useTestActions (액션 디스패처)</li>
              <li>• <strong>views/:</strong> RefStateMonitor, DynamicMountTest 등 + LogMonitor</li>
            </ul>
          </div>
          <div>
...
export function UseRefMountStateTestPage() {
  ...
  useRegisterSourceFile('pages/patterns/refs/UseRefMountStateTestPage.tsx', {
    ...
    description: 'Context-Layered Architecture로 구현된 useRefMountState 테스트 페이지',
