...
const {
  ...
  useActionDispatch: useWorkflowAction,
  useActionHandler: useWorkflowHandler,
} = createActionContext<WorkflowActions>('WorkflowSteps');

const { Provider: WorkflowStoreProvider, useStore: useWorkflowStore } =
  createStoreContext('WorkflowSteps', {
...
const WORKFLOW_STEPS = [
  ...
  },
  {
    ...
    description: 'Assign role-based permissions',
  },
  {
    id: 'notification',
    ...
    useCallback(
      async (payload, controller) => {
        ...
        try {
          ...
          if (step === 'validation') {
            const isValid =
              workflowData.userId && workflowData.email.includes('@');
            if (!isValid) {
            ...
            const permissions =
              workflowData.accountType === 'enterprise'
                ? ['read', 'write', 'admin']
      ...
      <div className="mb-8">
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-8">
          <p className="text-sm text-purple-800">
            <strong>Conditional Workflows:</strong> This pattern shows how
            actions can conditionally trigger subsequent steps based on previous
            results, creating dynamic multi-step processes.
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-4">
              {WORKFLOW_STEPS.map((step, index) => {
                ...
                const isCompleted =
                  index < currentStep ||
                  (workflowComplete && index === WORKFLOW_STEPS.length - 1);
                      ...
                      <div className="text-right">
                        {stepResult && (
                          <div className="text-sm">
                            <div
                              className={`font-medium ${stepResult.success ? 'text-green-600' : 'text-red-600'}`}
                            >
                              {stepResult.message}
                            </div>
            ...
            {workflowComplete && (
              <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                ...
                <p className="text-sm text-green-700 mt-1">
                  All steps completed successfully. User {workflowData?.userId}{' '}
          ...
          <div className="bg-purple-50 border border-purple-200 rounded-lg p-6">
            ...
            <div className="text-sm text-purple-800 space-y-2">
              <p>
                <strong>Step 1:</strong> Validation conditionally proceeds only
                if data is valid
              </p>
              <p>
                <strong>Step 2:</strong> Account setup conditionally adapts
                based on account type
              </p>
              <p>
                <strong>Step 3:</strong> Permissions conditionally assigned
                based on account tier
              </p>
              <p>
                <strong>Step 4:</strong> Notification conditionally sent if
                previous steps succeed
              </p>
              <p>
                <strong>Step 5:</strong> Completion conditionally triggered when
                all steps pass
              </p>
            </div>
          </div>
