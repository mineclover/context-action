...
const {
  Provider: FeatureActionProvider,
  useActionDispatch: useFeatureAction,
  useActionHandler: useFeatureHandler,
} = createActionContext<FeatureActions>('FeatureToggle');

const { Provider: FeatureStoreProvider, useStore: useFeatureStore } =
  createStoreContext('FeatureToggle', {
    features: {
      initialValue: {
        'advanced-analytics': {
...
function FeatureToggleContent() {
    ...
    useCallback(
      async (payload, controller) => {
        ...
        if (!feature.enabled) {
          ...
          feature.userGroup &&
          feature.userGroup !== 'all' &&
          feature.userGroup !== userContext.group
        ) {
        ...
        featureChecksStore.setValue(
          [checkResult, ...currentChecks].slice(0, 10)
        );
      },
    ...
    useCallback(
      async (payload, controller) => {
        ...
        if (!allowed) {
          ...
          executionLogsStore.setValue(
            [executionLog, ...currentLogs].slice(0, 10)
          );
  ...
  const handleExecuteFeature = useCallback(
    (featureKey: string) => {
      ...
      <div className="mb-8">
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-8">
          <p className="text-sm text-green-800">
            <strong>Conditional Features:</strong> This pattern demonstrates how
            actions can conditionally execute features based on environment
            settings, user groups, and feature flags.
          </p>
        </div>
      </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSelectedUser({
                      ...selectedUser,
                      group: e.target.value as UserContext['group'],
                    })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSelectedUser({
                      ...selectedUser,
                      environment: e.target.value as UserContext['environment'],
                    })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
        ...
        <div className="grid md:grid-cols-3 gap-4 text-sm">
          <div className="bg-white p-4 rounded">
            ...
            <p className="text-gray-700">
              Features conditionally execute based on deployment environment
              (dev/staging/prod)
            </p>
          </div>
          <div className="bg-white p-4 rounded">
            ...
            <p className="text-gray-700">
              Features conditionally available to specific user groups
              (beta/premium/all)
            </p>
          </div>
          <div className="bg-white p-4 rounded">
            ...
            <p className="text-gray-700">
              Global feature enable/disable controls conditional execution
            </p>
          </div>
        </div>
      </div>
