...
function StoreImmutabilityTestPage() {
    ...
    try {
      userStore.update((currentUser) => {
        ...
        try {
          ...
          addResult(
            `2단계 수정 성공: 나이가 ${currentUser.profile.age}으로 변경됨`
          );
        } catch (error) {
        ...
        try {
          ...
          addResult(
            `3단계 수정 성공: 도시가 '${currentUser.profile.address.city}'으로 변경됨`
          );
        } catch (error) {
      ...
      if (finalValue.name === 'Updated Name' && finalValue.profile.age === 26) {
        addResult(
          '✅ 추가 성공: update 함수의 반환값이 정상적으로 적용되었습니다!'
        );
      } else {
  ...
  const testDeepObjectProtection = () => {
    ...
    try {
      ...
      addResult(`⚠️ Immer 보호 동작: ${error}`);
      addResult(
        'ℹ️ 이는 Immer가 깊은 객체까지 완벽하게 보호하는 정상적인 동작입니다.'
      );
  ...
  const testImmerCopyOnWrite = () => {
    ...
    addResult(
      `변경 없음: 참조가 같은가? ${original === unchanged ? '✅ 예 (최적화됨)' : '❌ 아니오'}`
    );
    ...
    addResult(
      `변경 있음: 참조가 다른가? ${original !== changed ? '✅ 예 (새 객체)' : '❌ 아니오'}`
    );
    ...
    const addressRefShared =
      original.profile.address === partialChanged.profile.address;
    const hobbiesRefShared =
      original.profile.hobbies === partialChanged.profile.hobbies;

    addResult(
      `부분 변경 후 profile 참조 공유: ${profileRefShared ? '✅ 예' : '❌ 아니오'}`
    );
    addResult(
      `부분 변경 후 address 참조 공유: ${addressRefShared ? '✅ 예' : '❌ 아니오'}`
    );
    addResult(
      `부분 변경 후 hobbies 참조 공유: ${hobbiesRefShared ? '✅ 예' : '❌ 아니오'}`
    );
  ...
  const testPerformanceComparison = () => {
    ...
    addResult(
      `변경 시 JSON 대비: ${(jsonTime / immerChangeTime - 1).toFixed(1)}x 더 빠름`
    );
  };
      ...
      <div className="space-y-6">
        <div className="text-center">
          ...
          <p className="text-sm text-blue-600 bg-blue-50 px-4 py-2 rounded">
            ✨ <strong>Immer Copy-on-Write 최적화</strong>: 변경사항이 없으면
            원본 객체를 그대로 반환하여 성능을 최적화합니다.
          </p>
        </div>
        ...
        <DemoCard title="테스트 항목 설명" variant="default">
          <div className="space-y-4 text-sm">
            <div>
              ...
              <p className="text-gray-600 mb-2">
                <code>store.getValue()</code>로 받은 객체를{' '}
                <code>obj.name = "Modified Name"</code>,{' '}
                <code>obj.profile.age = 999</code> 등으로 직접 변경 시도
              </p>
              <div className="bg-green-50 border-l-4 border-green-400 p-2">
                <p className="text-green-700 text-xs">
                  <strong>✅ 예상 결과:</strong>{' '}
                  <code>TypeError: Cannot assign to read only property</code>{' '}
                  발생, Store 값은 <code>"John Doe"</code> 유지
                </p>
              </div>
              <div className="bg-red-50 border-l-4 border-red-400 p-2 mt-1">
                <p className="text-red-700 text-xs">
                  <strong>❌ 문제 상황:</strong> 에러 없이 수정되고 Store 값도{' '}
                  <code>"Modified Name"</code>으로 변경됨
                </p>
              </div>
            </div>
            <div>
              ...
              <p className="text-gray-600 mb-2">
                <code>store.setValue(obj)</code> 실행 후 원본 <code>obj</code>{' '}
                객체를 <code>obj.name = "Modified Jane"</code>으로 변경 시도
              </p>
              <div className="bg-green-50 border-l-4 border-green-400 p-2">
                <p className="text-green-700 text-xs">
                  <strong>✅ 예상 결과:</strong> 원본 객체는{' '}
                  <code>"Modified Jane"</code>으로 변경되지만 Store 값은{' '}
                  <code>"Jane Smith"</code> 유지
                </p>
              </div>
              <div className="bg-red-50 border-l-4 border-red-400 p-2 mt-1">
                <p className="text-red-700 text-xs">
                  <strong>❌ 문제 상황:</strong> Store 값도{' '}
                  <code>"Modified Jane"</code>으로 변경됨 (참조 공유 문제)
                </p>
              </div>
            </div>
            <div>
              ...
              <p className="text-gray-600 mb-2">
                <code>store.update(currentValue =&gt; ...)</code> 함수 내에서{' '}
                <code>currentValue.name = "Hacked Name"</code>,{' '}
                <code>currentValue.profile.age = 777</code> 등으로 직접 변경
                시도
              </p>
              <div className="bg-green-50 border-l-4 border-green-400 p-2">
                <p className="text-green-700 text-xs">
                  <strong>✅ 예상 결과:</strong>{' '}
                  <code>TypeError: Cannot assign to read only property</code>{' '}
                  발생, Store는 <code>return</code> 값인{' '}
                  <code>"Updated Name"</code>으로만 변경됨
                </p>
              </div>
              <div className="bg-red-50 border-l-4 border-red-400 p-2 mt-1">
                <p className="text-red-700 text-xs">
                  <strong>❌ 문제 상황:</strong> 직접 수정이 허용되어 Store 값이{' '}
                  <code>"Hacked Name"</code>으로 변경됨
                </p>
              </div>
              <div className="bg-blue-50 border-l-4 border-blue-400 p-2 mt-1">
                <p className="text-blue-700 text-xs">
                  <strong>ℹ️ Immer 동작:</strong> <code>draft</code> 객체는
                  Proxy로 변경을 추적하거나 차단
                  <br />
                  <strong>중요:</strong> 직접 수정이 허용되더라도 실제 Store에는{' '}
                  <code>return</code> 값만 반영됨
                </p>
              </div>
            </div>
            <div>
              ...
              <p className="text-gray-600 mb-2">
                <code>getValue()</code>로 받은 객체를{' '}
                <code>obj.profile.address.city = "Modified Tokyo"</code>,{' '}
                <code>obj.profile.hobbies.push("hacking")</code> 등으로 중첩
                변경 시도
              </p>
              <div className="bg-green-50 border-l-4 border-green-400 p-2">
                <p className="text-green-700 text-xs">
                  <strong>✅ 예상 결과:</strong> <code>TypeError</code>{' '}
                  발생하거나 Store 값은 <code>"Tokyo"</code>,{' '}
                  <code>["swimming", "traveling", "photography"]</code> 유지
                </p>
              </div>
              <div className="bg-red-50 border-l-4 border-red-400 p-2 mt-1">
                <p className="text-red-700 text-xs">
                  <strong>❌ 문제 상황:</strong> Store 값도{' '}
                  <code>"Modified Tokyo"</code>,{' '}
                  <code>["swimming", ..., "hacking"]</code>으로 변경됨
                </p>
              </div>
            </div>
            <div>
              ...
              <p className="text-gray-600 mb-2">
                변경사항 없는 <code>produce()</code> 실행과 부분 변경 시 참조
                공유 여부 확인
              </p>
              <div className="bg-green-50 border-l-4 border-green-400 p-2">
                <p className="text-green-700 text-xs">
                  <strong>✅ 예상 결과:</strong> 변경 없음 시{' '}
                  <code>original === result</code>, 부분 변경 시 미변경 부분은
                  참조 공유
                </p>
              </div>
              <div className="bg-red-50 border-l-4 border-red-400 p-2 mt-1">
                <p className="text-red-700 text-xs">
                  <strong>❌ 문제 상황:</strong> 항상 새 객체 생성하여 불필요한
                  메모리 사용
                </p>
              </div>
            </div>
            <div>
              ...
              <p className="text-gray-600 mb-2">
                1000회 반복으로 Immer Copy-on-Write vs{' '}
                <code>JSON.parse(JSON.stringify())</code> 방식의 실행 시간 측정
              </p>
              <div className="bg-green-50 border-l-4 border-green-400 p-2">
                <p className="text-green-700 text-xs">
                  <strong>✅ 예상 결과:</strong> Immer(변경 없음)가 JSON
                  방식보다 현저히 빠른 성능 (수십 배 차이)
                </p>
              </div>
              <div className="bg-red-50 border-l-4 border-red-400 p-2 mt-1">
                <p className="text-red-700 text-xs">
                  <strong>❌ 문제 상황:</strong> Immer가 더 느리거나 비슷한 성능
                  (최적화 실패)
                </p>
              </div>
            </div>
          ...
          <div className="space-y-3 text-sm">
            <div className="bg-blue-50 p-3 rounded">
              ...
              <p className="text-blue-700 mt-1">
                변경사항이 없으면 원본 객체를 그대로 반환하여 불필요한 복사를
                방지합니다. 메모리 사용량과 성능이 크게 개선됩니다.
              </p>
            </div>
            <div className="bg-green-50 p-3 rounded">
              ...
              <p className="text-green-700 mt-1">
                객체의 일부만 변경되면, 변경되지 않은 부분은 원본과 참조를
                공유합니다. 대용량 객체에서도 효율적인 업데이트가 가능합니다.
              </p>
            </div>
            <div className="bg-purple-50 p-3 rounded">
              <strong className="text-purple-800">불변성 보장</strong>
              <p className="text-purple-700 mt-1">
                모든 변경사항은 새로운 객체를 생성하므로 원본 데이터가 절대
                변경되지 않습니다. 예측 가능하고 안전한 상태 관리가 가능합니다.
              </p>
            </div>
          </div>
        </DemoCard>
