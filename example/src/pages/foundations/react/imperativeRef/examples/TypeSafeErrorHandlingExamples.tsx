...
export function TypeSafeErrorHandlingExample({
  children,
}: {
  ...
  const { errorState, reportError, recoverFromError, clearErrors } =
    useErrorRecovery();
  ...
  const handleSafeValidation = useCallback(async () => {
    try {
      ...
      if (!nameInput || !emailInput || !messageInput) {
        ...
        throw createRefError(`Missing refs: ${missingRefs}`, missingRefs);
      }
  ...
  return (
    <div className="bg-rose-50 p-4 rounded-lg border border-rose-200">
      ...
      {errorState.hasError && (
        <div className="mb-3 p-3 bg-red-100 border border-red-300 rounded">
          ...
          <div className="text-xs text-red-600 mt-1">
            Code: {errorState.error?.code} | Recoverable:{' '}
            {errorState.error?.recoverable ? 'Yes' : 'No'}
          </div>
        </div>
      )}
      ...
      <div className="mb-3 text-xs text-rose-600">
        Total Errors: {errorState.errorCount} | Status:{' '}
        {errorState.hasError ? 'Error State' : 'Healthy'}
      </div>
      ...
      <div className="text-xs bg-rose-100 p-2 rounded max-h-20 overflow-y-auto">
        {logs.length === 0 ? (
          ...
          logs.map((log, index) => (
            <div key={index} className="text-rose-700 font-mono">
              {log}
            </div>
          ))
        )}
...
export function CircuitBreakerExample({
  ...
  const executeWithCircuitBreaker = useCallback(
    async (operation: () => Promise<void> | void) => {
      ...
      if (
        circuitState === 'open' &&
        Date.now() - lastFailure > RECOVERY_TIMEOUT
      ) {
      ...
      try {
        ...
        if (newFailureCount >= FAILURE_THRESHOLD) {
          ...
          addLog(
            `🚨 Circuit breaker: opened after ${newFailureCount} failures`
          );
        } else {
          addLog(
            `❌ Operation failed (${newFailureCount}/${FAILURE_THRESHOLD})`
          );
        }
      }
    },
  ...
  const handleRiskyOperation = useCallback(() => {
    executeWithCircuitBreaker(async () => {
      ...
      const result = validateFormData(formData);
      addLog(
        `✅ Risky operation succeeded: ${result.isValid ? 'Valid' : 'Invalid'}`
      );
    });
    ...
    <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
      ...
      <div className="text-xs bg-orange-100 p-2 rounded max-h-20 overflow-y-auto">
        {logs.length === 0 ? (
          ...
          logs.map((log, index) => (
            <div key={index} className="text-orange-700 font-mono">
              {log}
            </div>
          ))
        )}
    ...
    <div className="space-y-6">
      <div className="bg-gray-50 p-4 rounded-lg border">
        ...
        <p className="text-sm text-gray-600 mb-4">
          고급 에러 처리 및 타입 안전성 패턴: 강건한 에러 핸들링, 회복 전략,
          회로 차단기 패턴
        </p>
      </div>
