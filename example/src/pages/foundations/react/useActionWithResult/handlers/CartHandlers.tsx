...
export interface CartHandlersProps {
  ...
  apiClient?: {
    ...
    applyDiscountCode?: (code: string) => Promise<{ valid: boolean; rate: number }>;
  };
  logger?: {
...
export function CartHandlers({
  ...
  useCartActionHandler('validateCart', useCallback(async (payload) => {
    try {
  ...
  useCartActionHandler('calculateTotal', useCallback(async (payload) => {
    try {
      ...
      if (discountCode && apiClient?.applyDiscountCode) {
        try {
          const discountValidation = await apiClient.applyDiscountCode(discountCode);
          if (!discountValidation.valid) {
            discountCode = undefined;
            logger?.info(`[${moduleId}] Invalid discount code: ${payload.discountCode}`);
          }
        } catch (error) {
      ...
      const calculationResult = calculateCartTotal(payload.items, discountCode);
  ...
  useCartActionHandler('processOrder', useCallback(async (payload) => {
    try {
      ...
      if (apiClient?.saveOrder) {
        try {
          ...
          logger?.info(`[${moduleId}] Order saved to API`, { orderId: orderResult.orderId });
        } catch (error) {
      ...
      if (orderResult.status === 'completed' && apiClient?.updateInventory) {
        try {
          await apiClient.updateInventory(payload.items);
          logger?.info(`[${moduleId}] Inventory updated`, { orderId: orderResult.orderId });
        } catch (error) {
  ...
  // 🎯 Clear Cart Handler
  useCartActionHandler('clearCart', useCallback(async () => {
    try {
      ...
      logger?.info(`[${moduleId}] Cart cleared`, { previousItemCount: itemCount });
      onCartCleared?.();

    } catch (error) {
      logger?.error(`[${moduleId}] Failed to clear cart`, error);
    }
  }, [moduleId, cartStore, validationStore, calculationStore, orderStore, logger, onCartCleared]));
