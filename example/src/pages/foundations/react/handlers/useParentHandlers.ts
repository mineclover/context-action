...
export function useParentCounterHandlers(props: ParentHandlerProps) {
  ...
  const incrementCounterHandler = useCallback(
    async (payload: void, controller: any) => {
...
export function useParentControlHandlers(props: ParentHandlerProps) {
  ...
  const requestChildControlHandler = useCallback(
    async (
      payload: {
        childId: string;
      ...
      if (enableLogging) {
        console.log(`🔄 [${moduleId}] 하위 컴포넌트 제어 요청:`, {
          childId,
          action,
          amount,
        });
...
export function useParentDataHandlers(props: ParentHandlerProps) {
  ...
  const childRegisteredHandler = useCallback(
    async (
      ...
      if (enableLogging) {
        console.log(`🔄 [${moduleId}] 하위 컴포넌트 등록:`, {
          childId,
          childType,
        });
  ...
  useEffect(() => {
    if (enableLogging) {
      console.log(
        `🔄 [${moduleId}] Parent Data Handlers registered - storeManager:`,
        !!storeManager
      );
    }
