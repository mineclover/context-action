...
import {
  useChildBActionDispatch,
  useChildBActionHandler,
  useChildBStoreManager,
...
export function useChildBTextActions() {
  ...
  const updateTextHandler = useCallback(
    async (payload: { newText: string }, _controller: any) => {
  ...
  // 핸들러 등록 - 명시적 ID 사용
  useChildBActionHandler('updateText', updateTextHandler, {
    id: 'child-b-text-update',
  });
  useChildBActionHandler('clearText', clearTextHandler, {
    id: 'child-b-text-clear',
  });

  // 리프레시 횟수 확인용 useEffect
  useEffect(() => {
    console.log(
      '🔄 ChildB Text Actions 리프레시됨 - storeManager, parentDispatch, childId 변경'
    );
  }, [storeManager, parentDispatch, childId]);

  // View에서 사용할 액션 함수들
  const updateText = useCallback(
    (newText: string) => childDispatch('updateText', { newText }),
    [childDispatch]
  );

  const clearText = useCallback(
    () => childDispatch('clearText', undefined),
    [childDispatch]
  );
