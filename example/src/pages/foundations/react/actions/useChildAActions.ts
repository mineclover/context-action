...
import { useChildAActionHandler, useChildAActionDispatch, useChildAStoreManager } from '../contexts/ChildAContext';
import { useParentActionDispatch, useParentActionHandler } from '../contexts/ParentContext';
...
export function useChildACounterActions() {
  ...
  // 핸들러 등록 - 명시적 ID 사용
  useChildAActionHandler('incrementCounter', incrementCounterHandler, { id: 'child-a-counter-increment' });
  useChildAActionHandler('resetCounter', resetCounterHandler, { id: 'child-a-counter-reset' });

  // 리프레시 횟수 확인용 useEffect
  useEffect(() => {
    console.log('🔄 ChildA Counter Actions 리프레시됨 - storeManager, childDispatch, parentDispatch, childId 변경');
  }, [storeManager, childDispatch, parentDispatch, childId]);

  // View에서 사용할 액션 함수들
  const incrementCounter = useCallback((amount: number) => 
    childDispatch('incrementCounter', { amount }), [childDispatch]);
  
  const resetCounter = useCallback(() => 
    childDispatch('resetCounter', undefined), [childDispatch]);
...
export function useChildARemoteControlActions() {
  ...
  const handleRemoteControl = useCallback(async (payload: { childId: string; action: 'increment' | 'reset'; amount?: number }, _controller: any) => {
    ...
    if (action === 'increment') {
      ...
      // 단순한 콘솔 로그 사용
      console.log('🎮 ChildA 원격 제어로 카운터 증가:', { amount: incrementAmount, newValue });
  ...
  useParentActionHandler('requestChildControl', handleRemoteControl, { id: `child-a-remote-control-${childId}` });

  // 리프레시 횟수 확인용 useEffect
  useEffect(() => {
    console.log('🔄 ChildA Remote Control Actions 리프레시됨 - storeManager, parentDispatch, childId 변경');
  }, [storeManager, parentDispatch, childId]);
