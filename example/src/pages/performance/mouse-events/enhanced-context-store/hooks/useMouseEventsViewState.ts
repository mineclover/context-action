...
export function useMouseEventsViewState() {
  ...
  const viewState = useMemo(() => ({
    ...
    position: {
      ...
      displayText: position.x !== -999 ? `(${position.x}, ${position.y})` : 'Outside'
    },
    
    // 움직임 정보
    movement: {
      ...movement,
      velocityText: movement.velocity > 0.1 
    ...
    activity: {
      ...
      statusColor: activity.current === 'moving' ? 'green' : 
                   activity.current === 'clicking' ? 'purple' : 'gray',
      isActive: activity.isInsideArea,
      timeSinceActivity: activity.lastActivity > 0 ? 
        Date.now() - activity.lastActivity : 0
    },
    ...
    performance: {
      ...
      memoryUsageText: performance.memoryUsage > 0 ? 
        `${(performance.memoryUsage / 1024 / 1024).toFixed(1)}MB` : 'N/A'
    }
  ...
  }), [movement.moveCount, clicks.total, activity.isInsideArea, computed.sessionDuration]);
