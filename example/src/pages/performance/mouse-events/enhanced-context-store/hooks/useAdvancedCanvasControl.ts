...
export function useAdvancedCanvasControl() {
  ...
  const createClickMarkerDirect = (x: number, y: number, timestamp: number) => {
    ...
    ripple.className =
      'absolute inset-0 w-8 h-8 border-2 border-purple-600 rounded-full animate-ping';
    ripple.style.animationDuration = '1000ms';

    // 클릭 마커
    const clickDot = document.createElement('div');
    clickDot.className =
      'w-8 h-8 bg-purple-500/20 border-2 border-purple-600 rounded-full flex items-center justify-center';
  ...
  const updatePathDirect = (newPoint: {
    x: number;
    y: number;
    timestamp: number;
  }) => {
