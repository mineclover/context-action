      ...
      <div className="page-container">
        <header className="page-header">
          ...
          <p className="page-description">
            ...
            단일 예시로 마우스 이벤트를 Action Pipeline을 통해 처리하고 실시간 피드백을 제공합니다.
          </p>
      ...
      <CardContent className="p-4 h-full flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-1">
          {eventLog.slice(-20).map((log) => (
            <div key={log.id} className="flex items-center gap-2 text-sm p-2 rounded">
...
function MouseEventsDemo() {
  ...
  const handleZoneEnter = useCallback((zoneName: string) => {
    dispatch('handleMouseEnter', { target: zoneName, timestamp: Date.now() });
  }, [dispatch]);

  const handleZoneLeave = useCallback((zoneName: string) => {
  ...
  return (
    <div className="space-y-6">
      {/* 통계 섹션 */}
      <section>
        <h2 className="text-xl font-semibold text-gray-800 mb-4">📊 실시간 통계</h2>
        <MouseStats 
      ...
      {/* 메인 상호작용 영역 */}
      <section>
        <h2 className="text-xl font-semibold text-gray-800 mb-4">🎯 상호작용 영역</h2>
            ...
            >
              ...
              <div 
                className={`absolute top-4 left-4 w-24 h-24 rounded-lg border-2 border-dashed transition-all duration-200 flex items-center justify-center text-sm font-medium ${
                  hoverZones['zone-1'] ? 'bg-green-200 border-green-400 text-green-700' : 'bg-gray-100 border-gray-300 text-gray-600'
                }`}
              ...
              <div 
                className={`absolute top-4 right-4 w-24 h-24 rounded-full border-2 border-dashed transition-all duration-200 flex items-center justify-center text-sm font-medium ${
                  hoverZones['zone-2'] ? 'bg-purple-200 border-purple-400 text-purple-700' : 'bg-gray-100 border-gray-300 text-gray-600'
                }`}
              ...
              <div 
                className={`absolute bottom-4 left-1/2 transform -translate-x-1/2 w-32 h-16 rounded-lg border-2 border-dashed transition-all duration-200 flex items-center justify-center text-sm font-medium ${
                  hoverZones['zone-3'] ? 'bg-blue-200 border-blue-400 text-blue-700' : 'bg-gray-100 border-gray-300 text-gray-600'
                }`}
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="text-center text-gray-600 bg-white/80 rounded-lg px-4 py-2 backdrop-blur-sm">
                  <div className="text-sm font-medium">마우스를 움직이거나 클릭해보세요</div>
                  <div className="text-xs text-gray-500 mt-1">호버 존에 마우스를 올려보세요</div>
                </div>
              </div>
            </div>
          </CardContent>
      ...
      {/* 이벤트 로그 */}
      <section>
        <h2 className="text-xl font-semibold text-gray-800 mb-4">📋 이벤트 로그</h2>
        <EventLogDisplay 
          eventLog={eventLog}
      ...
      {/* 코드 예시 */}
      <section>
        <h2 className="text-xl font-semibold text-gray-800 mb-4">💻 구현 예시</h2>
        <Card>
          <CardContent className="p-6">
            <div className="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
              <pre className="text-sm"><code>{`// Action Context 정의
