...
// ===== Store Context =====
const {
  Provider: ThrottleStoreProvider,
  useStore: useThrottleStore,
...
// ===== Action Context =====
const { Provider: ThrottleActionProvider, useActionDispatch, useActionHandler } = 
  ...
  return (
    <section className="bg-white rounded-2xl shadow-xl p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Advanced Throttle Comparison System</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="p-6 bg-gradient-to-r from-teal-50 to-cyan-50 rounded-xl">
            <h3 className="text-xl font-semibold text-teal-900 mb-4">🎯 System Architecture</h3>
            <div className="space-y-4 text-teal-800">
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-teal-500 rounded-full mt-2 flex-shrink-0" />
                <div>
                  <strong>Multiple Strategies:</strong> Normal, Throttle, Debounce, Leading Edge, Trailing Edge
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-cyan-500 rounded-full mt-2 flex-shrink-0" />
                <div>
                  <strong>Performance Tracking:</strong> 실시간 성능 메트릭스 및 효율성 분석
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 flex-shrink-0" />
                <div>
                  <strong>Comparative Analysis:</strong> 다양한 방식의 성능 및 리소스 사용량 비교
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl">
            <h3 className="text-xl font-semibold text-blue-900 mb-4">⚡ Throttling Strategies</h3>
        ...
        <div className="space-y-6">
          <div className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl">
            <h3 className="text-xl font-semibent text-green-900 mb-4">🔄 Event Flow</h3>
          ...
          <div className="p-6 bg-gradient-to-r from-orange-50 to-red-50 rounded-xl">
            <h3 className="text-xl font-semibold text-orange-900 mb-4">🛡️ Performance</h3>
  ...
  return (
    <section className="bg-white rounded-2xl shadow-xl p-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900">🎯 Interactive Throttle Comparison</h2>
...
function ThrottleDemoInterface() {
  ...
  useActionHandler('clearLogs', useCallback(async () => {
  ...
  useActionHandler('updateConfig', useCallback(async (payload) => {
    configStore.setValue(payload);
  }, [configStore]));

  useActionHandler('runPerformanceTest', useCallback(async (payload) => {
  ...
  useActionHandler('startAutoTest', useCallback(async (payload) => {
    ...
    autoTestInterval.current = setInterval(() => {
      ...
      ['normal', 'throttle', 'debounce', 'leading', 'trailing'].forEach(type => {
  ...
  useActionHandler('stopAutoTest', useCallback(async () => {
  ...
  const handleEventByType = useCallback((type: string, value: string) => {
  ...
  return (
    <div className="space-y-8">
            ...
            <div key={key}>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => dispatch('updateConfig', {
                    ...config,
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl">
          <h3 className="text-lg font-semibold text-green-900 mb-4">🚀 자동 테스트</h3>
          <div className="space-y-3">
            <button
              onClick={() => dispatch('startAutoTest', { duration: 20, frequency: 100 })}
            ...
            >
              {isAutoTesting ? <span className="w-4 h-4" /> : <span className="w-4 h-4" />}
              {isAutoTesting ? '테스트 중...' : '자동 이벤트 생성'}
            </button>
            
            <button
              onClick={() => dispatch('runPerformanceTest', { type: 'normal', iterations: 50, interval: 20 })}
        ...
        <div className="p-6 bg-gradient-to-r from-orange-50 to-red-50 rounded-xl">
          <h3 className="text-lg font-semibold text-orange-900 mb-4">🛠️ 관리 도구</h3>
          <div className="space-y-3">
            <button
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          { key: 'normal', label: '일반', color: 'red', icon: () => <span>🔴</span> },
          { key: 'throttle', label: '쓰로틀', color: 'blue', icon: () => <span>🔵</span> },
          { key: 'debounce', label: '디바운스', color: 'green', icon: () => <span>🟢</span> },
          { key: 'leading', label: '리딩', color: 'purple', icon: () => <span>🟣</span> },
          { key: 'trailing', label: '트레일링', color: 'orange', icon: () => <span>🟠</span> }
        ].map(({ key, label, color, icon: Icon }) => (
          <div key={key} className={`bg-${color}-50 p-4 rounded-lg`}>
            <div className="flex items-center justify-between mb-2">
              <Icon />
              <span className="text-xs text-gray-600">{(config?.[`${key}Delay` as keyof typeof config] || 0)}ms</span>
            </div>
            ...
            eventLogs.map((log) => (
              <div
                ...
                className={`p-3 rounded-lg border-l-4 ${
                  log.type === 'normal' ? 'bg-red-50 border-l-red-500' :
                  log.type === 'throttle' ? 'bg-blue-50 border-l-blue-500' :
                  log.type === 'debounce' ? 'bg-green-50 border-l-green-500' :
                  log.type === 'leading' ? 'bg-purple-50 border-l-purple-500' :
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-1 rounded text-xs font-medium text-white ${
                      log.type === 'normal' ? 'bg-red-500' :
                      log.type === 'throttle' ? 'bg-blue-500' :
                      log.type === 'debounce' ? 'bg-green-500' :
                      log.type === 'leading' ? 'bg-purple-500' :
                      'bg-orange-500'
                    }`}>
...
function StatusSection() {
  ...
  const totalEvents = useMemo(() => 
    Object.values(metrics?.eventCounts || {}).reduce((sum, count) => sum + count, 0),
    [metrics?.eventCounts]
  );
  ...
  return (
    <section className="bg-white rounded-2xl shadow-xl p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Throttle Performance Analytics</h2>
          <p className="text-gray-600">실시간 쓰로틀링 성능 분석 및 비교 메트릭스</p>
        </div>
      </div>
          ...
          <div className="p-6 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-blue-900">{Math.round(averageEfficiency)} ev/s</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-blue-700">테스트 상태</span>
                <span className={`font-bold ${isAutoTesting ? 'text-green-600' : 'text-gray-600'}`}>
          ...
          <div className="p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-purple-900">{Math.round(metrics?.avgResponseTimes.normal || 0)}ms</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-purple-700">쓰로틀</span>
                <span className="font-bold text-purple-900">{Math.round(metrics?.avgResponseTimes.throttle || 0)}ms</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-purple-700">디바운스</span>
                <span className="font-bold text-purple-900">{Math.round(metrics?.avgResponseTimes.debounce || 0)}ms</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 bg-gradient-to-r from-teal-50 to-cyan-50 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-teal-900">{config?.throttleDelay}ms</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-teal-700">디바운스 지연</span>
                <span className="font-bold text-teal-900">{config?.debounceDelay}ms</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-teal-700">리딩 지연</span>
                <span className="font-bold text-teal-900">{config?.leadingDelay}ms</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full ${isAutoTesting ? 'bg-blue-500 animate-pulse' : 'bg-gray-400'}`} />
                <span className="text-green-700">자동 테스트 {isAutoTesting ? '실행중' : '대기중'}</span>
              </div>
              <div className="flex items-center gap-2">
          ...
          <div className="p-6 bg-gradient-to-r from-gray-50 to-slate-50 rounded-xl">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">📊 효율성 차트</h3>
            <div className="space-y-3 text-sm">
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-gray-700">전체 효율성</span>
                  <span className="text-gray-900 font-medium">{Math.round(averageEfficiency)}%</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-green-500 h-2 rounded-full" 
                    style={{ width: `${Math.min(100, (averageEfficiency / 100) * 100)}%` }}
                  />
                </div>
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-gray-900 font-medium">
                    {totalEvents > 0 ? Math.round((metrics?.eventCounts.throttle || 0) / totalEvents * 100) : 0}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div 
...
{`const { Provider, useStore } = createStoreContext('AdvancedThrottle', {
  ...
  metrics: {
    ...
    cpuLoad: { normal: 0, throttle: 0, debounce: 0, leading: 0, trailing: 0 }
          ...
          <div className="p-6 bg-gray-50 rounded-xl">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">⏱️ Throttle Implementation</h3>
            <pre className="text-sm bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
{`function useThrottle<T extends (...args: any[]) => void>(
  func: T, 
  delay: number, 
  ...
  return useCallback(((...args: Parameters<T>) => {
    ...
    if (leading && now - lastExecutedRef.current >= delay) {
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        <div className="space-y-6">
          <div className="p-6 bg-gray-50 rounded-xl">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">🎯 Debounce Implementation</h3>
            <pre className="text-sm bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
    ...
    efficiency: {
      ...
      [payload.type]: newEventCount > 0 ? (1000 / newAvg) : 0
          ...
          <div className="p-6 bg-teal-50 rounded-xl">
            <h3 className="text-lg font-semibold text-teal-900 mb-3">🔧 최적화 가이드라인</h3>
