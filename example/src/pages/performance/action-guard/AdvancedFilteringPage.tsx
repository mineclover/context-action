...
const { Provider: DemoStoreProvider, useStore: useDemoStore } =
            ...
            <Fragment key={handler.id}>
              ...
              {index < handlers.length - 1 && (
                <div
                  ...
                  ${
                    isExecuted &&
                    executedHandlers.includes(handlers[index + 1]?.id || '')
...
function FilteringDemo() {
  ...
  const runDemo = useCallback(
    async (demoKey: string, filterOptions?: any) => {
      ...
      try {
        ...
        console.log(
          '✅ [runDemo] ActionRegister fix successful! Handler tracking working:',
          {
            executed,
            executedHandlers,
            handlersData: result.handlers?.map((h) => ({
              id: h.id,
              executed: h.executed,
            })),
  ...
  const demos = [
    ...
    },
    {
      ...
      filterOptions: {
        filter: { handlerIds: ['analytics', 'notification', 'audit-log'] },
      },
      category: 'Handler IDs',
    },
    ...
    },
    {
      ...
      filterOptions: {
        filter: {
          custom: (config: any) =>
            ['security-check', 'database-save', 'audit-log'].includes(
              config.id
            ),
        },
      },
  ...
  const demosByCategory = demos.reduce(
    (acc, demo) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {categoryDemos.map((demo) => (
              ...
              >
                ...
                >
                  {isLoading && results[demo.key] === null
                    ? 'Running...'
                    : 'Execute'}
                </button>
                    ...
                    {results[demo.key]?.error ? (
                      ...
                      <div className="space-y-1">
                        <div className="text-green-600 text-sm flex items-center gap-1">
                          <span>✅</span>
                          <span>
                            {results[demo.key]?.execution?.handlersExecuted ||
                              0}{' '}
                            handlers executed
                          </span>
                        </div>
                        <div className="text-blue-600 text-sm flex items-center gap-1">
                          <span>⚡</span>
                          <span>
                            {results[demo.key]?.execution?.duration || 0}ms
                            duration
                          </span>
                        </div>
            ...
            <div className="bg-white rounded-lg p-3">
              <div className="text-2xl font-bold text-blue-600">
                {Object.values(results).reduce(
                  (sum, r) => sum + (r?.execution?.handlersExecuted || 0),
                  0
                )}
              </div>
          ...
          <div className="container mx-auto px-4 py-8">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 mb-6">
                Real-time handler execution visualization with state-based
                tracking
              </p>
                    ...
                    return (
                      <div
                        ...
                        className={`bg-white border-2 rounded-lg p-3 text-center ${
                          handlerConfig.color === 'red'
                            ? 'border-red-200'
                            : handlerConfig.color === 'blue'
                              ? 'border-blue-200'
                              : handlerConfig.color === 'green'
                                ? 'border-green-200'
                                : handlerConfig.color === 'yellow'
                                  ? 'border-yellow-200'
                                  : 'border-purple-200'
                        }`}
                      >
