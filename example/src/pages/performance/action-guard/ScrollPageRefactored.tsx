...
interface ScrollActions {
  updateScrollPosition: { scrollTop: number; scrollLeft: number; element: string; velocity: number; direction: 'up' | 'down' };
...
const priorityColors = [
  'hsl(220, 70%, 88%)', 'hsl(180, 70%, 88%)', 'hsl(140, 70%, 88%)', 
  'hsl(60, 70%, 88%)', 'hsl(300, 70%, 88%)'
];
  ...
  return (
    <section className="bg-white rounded-2xl shadow-xl p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Advanced Scroll System</h1>
          <p className="text-gray-600">무한 스크롤과 가상화된 스크롤링 시스템</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="p-6 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl">
            <h3 className="text-xl font-semibold text-purple-900 mb-4">🎯 System Architecture</h3>
            <div className="space-y-4 text-purple-800">
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0" />
                <div>
                  <strong>Action Pipeline:</strong> updateScrollPosition, reachScrollEnd, loadMoreContent, smoothScrollTo
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-indigo-500 rounded-full mt-2 flex-shrink-0" />
                <div>
                  <strong>Store Management:</strong> scrollData, content, loading, virtualization, metrics
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-2 h-2 bg-blue-500 rounded-full mt-2 flex-shrink-0" />
                <div>
                  <strong>Virtualization:</strong> 메모리 효율적인 대용량 데이터 렌더링
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="p-6 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl">
            <h3 className="text-xl font-semibold text-blue-900 mb-4">🔄 Scroll Flow</h3>
  ...
  return (
    <section className="bg-white rounded-2xl shadow-xl p-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900">🎯 Interactive Scroll Demo</h2>
...
function ScrollDemoInterface() {
  ...
  useActionHandler('reachScrollEnd', useCallback(async (payload) => {
  ...
  useActionHandler('loadMoreContent', useCallback(async (payload) => {
    ...
    const newContent = generateScrollContent(payload.page, payload.itemsPerPage);
    ...
    metricsStore.setValue({
      ...
      performanceScore: Math.max(90, currentMetrics.performanceScore - (loadTime > 1000 ? 1 : 0))
    });
  ...
  useActionHandler('setAutoScroll', useCallback(async (payload) => {
    autoScrollingStore.setValue(payload.enabled);
    scrollSpeedStore.setValue(payload.speed);
  }, [autoScrollingStore, scrollSpeedStore]));

  useActionHandler('updateVirtualization', useCallback(async (payload) => {
  ...
  const handleScroll = useCallback((event: React.UIEvent<HTMLDivElement>) => {
    ...
    const endIndex = Math.min(startIndex + Math.ceil(clientHeight / itemHeight) + 2, content.length);
  ...
  const scrollStats = useMemo(() => {
    ...
    return {
      ...
      visibleItems: (virtualization?.endIndex || 0) - (virtualization?.startIndex || 0),
      scrollPercentage,
        ...
        <div className="grid md:grid-cols-4 gap-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              <button
                onClick={() => dispatch('smoothScrollTo', { element: 'scroll-container', target: 0, direction: 'y' })}
              ...
              <button
                onClick={() => {
                  ...
                  if (container) {
                    ...
                    dispatch('smoothScrollTo', { element: 'scroll-container', target: middle, direction: 'y' });
                  }
                }}
              ...
              <button
                onClick={() => {
                  ...
                  if (container) {
                    ...
                    dispatch('smoothScrollTo', { element: 'scroll-container', target: bottom, direction: 'y' });
                  }
                }}
            ...
            <div className="space-y-2">
              <div>
                ...
                <input
                  ...
                  max="10"
                  value={scrollSpeed}
                  onChange={(e) => dispatch('setAutoScroll', { enabled: autoScrolling, speed: parseInt(e.target.value) })}
                  className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer"
                />
              </div>
              
              <button
                onClick={() => dispatch('setAutoScroll', { enabled: !autoScrolling, speed: scrollSpeed })}
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={() => {
                  ...
                  dispatch('smoothScrollTo', { element: 'scroll-container', target: 0, direction: 'y' });
                }}
          ...
          <div>
            ...
            <div className="space-y-1 text-xs text-gray-600">
              ...
              <div>방향: {scrollStats.direction === 'down' ? '아래' : '위'}</div>
            ...
            {content?.slice(
              ...
              <div 
                ...
                style={{ 
                  ...
                  marginTop: index === 0 ? `${(virtualization?.startIndex || 0) * (virtualization?.itemHeight || 100)}px` : '0'
                }}
              >
            ...
            {loading && (
              <div className="flex justify-center items-center py-8">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-600">새로운 컨텐츠를 로드하는 중...</span>
                </div>
              </div>
            )}
          </div>
  ...
  return (
    <section className="bg-white rounded-2xl shadow-xl p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Scroll Performance Metrics</h2>
          ...
          <div className="p-6 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-blue-900">{metrics?.totalScrolls || 0}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-blue-700">최대 깊이</span>
                <span className="font-bold text-blue-900">{Math.round(metrics?.maxScrollDepth || 0)}px</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-blue-700">현재 속도</span>
                <span className="font-bold text-blue-900">{Math.round((scrollData?.velocity || 0) * 1000)} px/s</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-purple-900">{content.length}</span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-purple-900">
                  {content.length > 0 ? Math.round(((virtualization?.endIndex || 0) - (virtualization?.startIndex || 0)) / content.length * 100) : 0}%
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl">
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-green-900">{metrics?.performanceScore || 95}%</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-green-700">로드된 페이지</span>
                <span className="font-bold text-green-900">{metrics?.loadedPages || 1}</span>
              </div>
              <div className="flex justify-between items-center">
...
{`const { Provider, useStore } = createStoreContext('AdvancedScroll', {
  ...
  metrics: {
    ...
    performanceScore: 95
          ...
          <div className="p-6 bg-gray-50 rounded-xl">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">🎯 Virtualization Logic</h3>
            <pre className="text-sm bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
{`const handleScroll = useCallback((event: React.UIEvent<HTMLDivElement>) => {
  const target = event.currentTarget;
  const scrollTop = target.scrollTop;
  ...
  if (scrollHeight - scrollTop - clientHeight < 200) {
    dispatch('reachScrollEnd', {
      ...
      direction: 'bottom'
        ...
        <div className="space-y-6">
          <div className="p-6 bg-gray-50 rounded-xl">
            <h3 className="text-xl font-semibold text-gray-900 mb-4">⚡ Smooth Scroll Animation</h3>
            <pre className="text-sm bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
{`useActionHandler('smoothScrollTo', useCallback(async (payload) => {
  const container = scrollContainerRef.current;
  if (!container) return;
  ...
  metricsStore.setValue({
    ...
    maxScrollDepth: Math.max(currentMetrics.maxScrollDepth, payload.scrollTop)
          ...
          <div className="p-6 bg-purple-50 rounded-xl">
            <h3 className="text-lg font-semibold text-purple-900 mb-3">🔧 Key Features</h3>
