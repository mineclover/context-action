...
export function useApiManager(): UseApiManagerReturn {
  ...
  const execute = useCallback(
    async <T>(config: ApiRequestConfig): Promise<ApiResponse<T>> => {
      const startTime = Date.now();
      const cacheKey =
        config.cacheKey ||
        `${config.method}-${config.url}-${JSON.stringify(config.body)}`;
...
export function usePriorityExecution(): UsePriorityExecutionReturn {
  ...
  const activeActionsRef = useRef<Map<string, ActionPerformanceData>>(
    new Map()
  );
  ...
  usePerformanceHandler(
    ...
    useCallback(
      async (payload) => {
        ...
        console.log(
          `✅ Action completed: ${actionData.actionType} in ${duration}ms`
        );
      },
      [dispatch]
    )
  ...
  const executeWithPriority = useCallback(
    async <T = unknown, P = unknown>(
        ...
        await dispatch('failActionExecution', {
          ...
          error:
            error instanceof Error
              ? error
              : new Error('Unknown execution error'),
          duration:
            Date.now() -
            (activeActionsRef.current.get(actionId)?.startTime || Date.now()),
        });
  ...
  const getMetricsByType = useCallback(
    (actionType: string): PerformanceMetrics[] => {
