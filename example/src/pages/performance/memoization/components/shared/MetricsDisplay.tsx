...
export function MetricsDisplay({ 
  ...
  const borderColor = variant === 'memoized' ? 'border-green-500' : 'border-red-500';
    ...
    <div className={`p-4 border-2 rounded-lg ${borderColor} ${bgColor}`}>
      ...
      <div className="mb-3 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className={`font-mono font-bold ${getRenderRateColor(metrics.renderRateStatus)}`}>
...
function getRenderRateColor(status: 'normal' | 'warning' | 'danger' | 'critical') {
  switch (status) {
...
function getHeavyDataColor(status: 'normal' | 'heavy' | 'blocked') {
  switch (status) {
    ...
    case 'heavy': return 'text-yellow-600 font-bold';
    default: return '';
  }
}
...
function getMemoryDataColor(status: 'normal' | 'warning' | 'leak' | 'blocked') {
  switch (status) {
    case 'blocked': return 'text-red-600 font-bold';
    case 'leak': return 'text-red-600 font-bold';
