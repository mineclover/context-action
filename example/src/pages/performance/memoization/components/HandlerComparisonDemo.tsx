...
const { Provider: ComparisonStoreProvider, useStore: useComparisonStore } =
    ...
    async (payload: { dataSize: number }) => {
      ...
      store.setValue({
        ...
        processedResults: [
          ...
          ...result.map((v, i) => ({ id: i, value: v, timestamp: Date.now() })),
        ],
      });
    },
    [expensiveCalculator]
  ); // expensiveCalculator에 의존
    ...
    <div className="p-4 border-2 border-green-500 rounded-lg bg-green-50">
      ...
      <div className="mb-3 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span
            className={`font-mono font-bold ${renderRate > 5 ? 'text-yellow-600' : 'text-green-600'}`}
          >
    ...
    <div className="p-4 border-2 border-red-500 rounded-lg bg-red-50">
      ...
      <div className="mb-3 text-sm">
        ...
        <div className="flex justify-between">
          <span>Render Rate:</span>
          <span
            className={`font-mono font-bold ${renderRate > 10 ? 'text-red-600' : renderRate > 5 ? 'text-yellow-600' : 'text-green-600'}`}
          >
            {renderRate.toFixed(1)}/sec
          </span>
        </div>
      ...
      <div className="mb-3 p-2 bg-white rounded text-xs space-y-1">
        ...
        <div>Calc Result: {value.calcResult}</div>
        <div
          className={`${value.heavyData.length > 5000 ? 'text-red-600 font-bold' : value.heavyData.length > 2000 ? 'text-yellow-600 font-bold' : ''}`}
        >
          Heavy Data: {value.heavyData.length} items
          {value.heavyData.length > 5000 && ' 🚨 BLOCKED!'}
          {value.heavyData.length > 2000 &&
            value.heavyData.length <= 5000 &&
            ' 🔥 HEAVY!'}
        </div>
        <div>Processed: {value.processedResults.length} results</div>
        <div
          className={`${value.memoryLeakData.length > 1000 ? 'text-red-600 font-bold' : value.memoryLeakData.length > 500 ? 'text-yellow-600 font-bold' : ''}`}
        >
          Memory Data: {value.memoryLeakData.length} objects
          {value.memoryLeakData.length > 5000 && ' 🚨 BLOCKED!'}
          {value.memoryLeakData.length > 1000 &&
            value.memoryLeakData.length <= 5000 &&
            ' 🔥 LEAK!'}
          {value.memoryLeakData.length > 500 &&
            value.memoryLeakData.length <= 1000 &&
            ' ⚠️ WARNING'}
        </div>
      </div>
    ...
    if (isActive) {
      intervalRef.current = setInterval(() => {
        ...
        if (random < 0.25) {
          ...
          dispatch('complexCalculation', {
            multiplier: Math.floor(Math.random() * 20),
          });
        } else {
          dispatch('heavyOperation', {
            dataSize: Math.floor(Math.random() * 50) + 10,
          });
        }
      }, interval);
    } else {
...
function NonMemoizedAutoUpdater({
  dispatch,
  isActive,
  interval,
}: {
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          <div className="space-y-3">
            <div>
              ...
              <ul className="text-gray-600 space-y-1 ml-4">
                <li>
                  🎯 <strong>함수 재사용:</strong> 핸들러 함수는 한 번만 생성
                </li>
                <li>
                  🔄 <strong>지연 평가:</strong> 데이터는 실행 시점에 최신값
                  획득
                </li>
                <li>
                  💰 <strong>계산 최적화:</strong> 복잡한 로직도 메모이제이션
                </li>
                <li>
                  📈 <strong>낮은 렌더링:</strong> 불필요한 재등록 방지
                </li>
                <li>
                  🧹 <strong>메모리 효율:</strong> 가비지 컬렉션 부하 감소
                </li>
              </ul>
            </div>
          </div>
          <div className="space-y-3">
            <div>
              ...
              <ul className="text-gray-600 space-y-1 ml-4">
                <li>
                  💥 <strong>함수 재생성:</strong> 렌더링마다 모든 함수 새로
                  생성
                </li>
                <li>
                  💸 <strong>계산 중복:</strong> 복잡한 로직도 매번 재정의
                </li>
                <li>
                  📊 <strong>메모리 누수:</strong> 데이터 계속 누적 + 순환참조
                </li>
                <li>
                  🔥 <strong>높은 렌더링:</strong> 핸들러 재등록으로 성능 저하
                </li>
                <li>
                  🗑️ <strong>GC 부하:</strong> 대량 객체 생성으로 버벅임
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="mt-4 p-3 bg-yellow-100 border border-yellow-300 rounded">
          <div className="text-sm text-yellow-800">
            ...
            <div className="mt-2 grid md:grid-cols-2 gap-2">
              <div>
                <strong>1단계:</strong> 기본 버튼들로 차이 체험
                <br />
                <strong>2단계:</strong> Heavy 버튼으로 성능 차이 확인
                <br />
                <strong>3단계:</strong> Memory 버튼으로 누수 현상 관찰
              </div>
              <div>
                <strong>고급:</strong> Auto Update로 지속적 테스트
                <br />
                <strong>관찰:</strong> 🔥 LEAK!, 🚨 BLOCKED! 상태 변화
                <br />
