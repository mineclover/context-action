...
export function useRerenderMonitor(
  ...
  useEffect(() => {
    ...
    const recentRenders = renderTimesRef.current.filter(
      (_, index) => {
        const renderTime = now - renderTimesRef.current
          .slice(index)
  ...
  const getComparison = () => {
    ...
    return {
      ...
      difference: {
        ...
        averageTime: Math.abs(metrics1.averageRenderTime - metrics2.averageRenderTime)
      },
