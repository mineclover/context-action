...
function CorePerformanceContent() {
  ...
  const actionRegisterRef = useRef<ActionRegister<PerformanceActionMap> | null>(null);
  ...
  useEffect(() => {
    ...
    // 기본 카운터 핸들러
    actionRegister.register('incrementCounter', (counterId) => {
    ...
    // 배치 증가 핸들러
    actionRegister.register('batchIncrement', ({ startId, count }) => {
      const [, executionTime] = measureExecutionTime(() => {
        setCounters(prev => {
    ...
    // 스트레스 테스트 핸들러
    actionRegister.register('stressTest', async ({ iterations, concurrent }) => {
      setIsRunning(true);
      setTestProgress(0);
      ...
      const result: BenchmarkResult = {
        testName: concurrent ? 'Concurrent Stress Test' : 'Sequential Stress Test',
        iterations,
        totalTime,
        averageTime: avgTime,
        opsPerSecond,
      ...
      setMetrics(prev => ({
        ...prev,
        lastBenchmarkResults: [result, ...prev.lastBenchmarkResults].slice(0, 5),
      }));
    ...
    // 메모리 테스트 핸들러
    actionRegister.register('memoryTest', ({ objectCount }) => {
      const [, executionTime] = measureExecutionTime(() => {
        // 대량의 객체 생성 및 저장
    ...
    // 구독 테스트 핸들러
    actionRegister.register('subscriptionTest', ({ handlerCount }) => {
      const [, executionTime] = measureExecutionTime(() => {
        // 동적으로 다수의 핸들러 등록
    ...
    // 정리 핸들러
    actionRegister.register('cleanupTest', () => {
      const [, executionTime] = measureExecutionTime(() => {
  ...
  const dispatch = (action: keyof PerformanceActionMap, payload?: PerformanceActionMap[keyof PerformanceActionMap]) => {
  ...
  return (
    <div>
      ...
      <p>
        성능 최적화와 벤치마크를 다룹니다: 대량 액션 처리, 메모리 효율성, 실행 속도 측정, 
      ...
      <div style={{ 
        ...
        <div style={{ padding: '15px', border: '2px solid #007bff', borderRadius: '8px', textAlign: 'center' }}>
          <h4 style={{ margin: '0 0 10px 0', color: '#007bff' }}>📊 Total Actions</h4>
          <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{metrics.actionCount}</div>
        </div>

        <div style={{ padding: '15px', border: '2px solid #28a745', borderRadius: '8px', textAlign: 'center' }}>
          <h4 style={{ margin: '0 0 10px 0', color: '#28a745' }}>⚡ Avg Time</h4>
          <div style={{ fontSize: '24px', fontWeight: 'bold' }}>
            {metrics.averageExecutionTime.toFixed(3)}ms
          </div>
        </div>

        <div style={{ padding: '15px', border: '2px solid #ffc107', borderRadius: '8px', textAlign: 'center' }}>
        ...
        <div style={{ padding: '15px', border: '2px solid #17a2b8', borderRadius: '8px', textAlign: 'center' }}>
          <h4 style={{ margin: '0 0 10px 0', color: '#17a2b8' }}>🎯 Handlers</h4>
          <div style={{ fontSize: '24px', fontWeight: 'bold' }}>{metrics.handlerCount}</div>
        </div>
      </div>
      ...
      <div style={{ 
        ...
        <div style={{ padding: '20px', border: '1px solid #e9ecef', borderRadius: '8px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <button
              onClick={() => dispatch('incrementCounter', Math.floor(Math.random() * 10))}
              style={{
                padding: '8px 16px',
            ...
            <button
              onClick={() => dispatch('batchIncrement', { startId: 0, count: 100 })}
              style={{
                padding: '8px 16px',
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8f9fa' }}>
                  <th style={{ padding: '8px', border: '1px solid #dee2e6', textAlign: 'left' }}>Test Name</th>
                  <th style={{ padding: '8px', border: '1px solid #dee2e6', textAlign: 'right' }}>Iterations</th>
                  <th style={{ padding: '8px', border: '1px solid #dee2e6', textAlign: 'right' }}>Total Time</th>
                  <th style={{ padding: '8px', border: '1px solid #dee2e6', textAlign: 'right' }}>Avg Time</th>
                  <th style={{ padding: '8px', border: '1px solid #dee2e6', textAlign: 'right' }}>Ops/Sec</th>
              ...
              <tbody>
                {metrics.lastBenchmarkResults.map((result, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ padding: '8px', border: '1px solid #dee2e6' }}>{result.timestamp}</td>
                  </tr>
                ))}
              </tbody>
            </table>
