...
import { CanvasShape, Point, useCanvas } from './CanvasContext';
  ...
  const handleMouseUp = useCallback(
    (event: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (currentMode === 'select' && dragShape) {
        ...
        if (currentTool !== 'freehand') {
          if (
            Math.abs(pos.x - dragStart.x) > 5 ||
            Math.abs(pos.y - dragStart.y) > 5
          ) {
          ...
          const newShape: CanvasShape = {
            ...
            width:
              Math.max(...freehandPoints.map((p) => p.x)) -
              Math.min(...freehandPoints.map((p) => p.x)),
            height:
              Math.max(...freehandPoints.map((p) => p.y)) -
              Math.min(...freehandPoints.map((p) => p.y)),
            color: currentColor,
            strokeWidth: strokeWidth,
