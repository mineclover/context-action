...
import {
  ConcurrentActionsProvider,
  type TaskStatus,
  useConcurrentAction,
  useConcurrentActionWithResult,
} from './contexts/ConcurrentActionsContexts';
import { ConcurrentActionHandlerRegistry } from './handlers/ConcurrentActionHandlerRegistry';

function ConcurrentActionTestPageContent() {
  const [tasks, setTasks] = useState<TaskStatus[]>([]);
  const [taskControllers, setTaskControllers] = useState<
    Map<string, AbortController>
  >(new Map());

  const dispatch = useConcurrentAction();
  const { abortAll } = useConcurrentActionWithResult();
  ...
  const updateTaskStatus = useCallback(
    (taskId: string, status: TaskStatus['status'], error?: string) => {
      setTasks((prev) =>
        prev.map((task) => {
          if (task.id === taskId) {
  ...
  const addNetworkTask = useCallback((taskId: string, name: string) => {
    setTasks((prev) => [
      ...prev,
      {
        id: taskId,
  ...
  const addBackgroundTask = useCallback((taskId: string, name: string) => {
    setTasks((prev) => [
      ...prev,
      {
        id: taskId,
  ...
  return (
    <ConcurrentActionHandlerRegistry
      updateTaskProgress={updateTaskProgress}
      updateTaskStatus={updateTaskStatus}
      addNetworkTask={addNetworkTask}
            ...
            {summary.total > 0 && (
              <div className="mt-3 pt-3 border-t border-gray-200">
                <div className="text-center">
                  ...
                  <span
                    className={`font-bold ${
                      (summary.completed / summary.total) >= 0.8
                        ? 'text-green-600'
                        : summary.completed / summary.total >= 0.6
                          ? 'text-yellow-600'
                          : 'text-red-600'
                    }`}
                  >
                ...
                >
                  <div className="flex-1">
                    <div className="font-medium">
                      {task.name === 'longRunningTaskA'
                        ? '📊 Data Processing Task'
                        : task.name === 'longRunningTaskB'
                          ? '🌐 Network Synchronization'
                          : task.name === 'quickTask'
                            ? '✅ Quick Validation'
                            : task.name === 'networkRequest'
                              ? '🔗 API Network Request'
                              : task.name === 'backgroundJob'
                    ...
                    <div className="text-sm text-gray-500">
                      ...
                      {task.estimatedDuration && (
                        <div className="text-xs mt-1">
                          Expected: {(task.estimatedDuration / 1000).toFixed(1)}
                          s
                        </div>
                      )}
                    </div>
