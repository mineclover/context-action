...
export function CanvasToolbar({ onFocusCanvas, onTestAction }: CanvasToolbarProps) {
  ...
  const handleToolChange = useCallback((tool: 'rectangle' | 'circle' | 'line' | 'freehand') => {
    setTool(tool);
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-2 min-w-[160px]">
          ...
          <div className="flex items-center gap-2 flex-1">
            <span className={`text-sm font-bold w-6 transition-colors ${
              strokeWidth <= 5 ? 'text-green-600' :
              strokeWidth <= 10 ? 'text-yellow-600' :
              'text-red-600'
            }`}>
              {strokeWidth}
            </span>
