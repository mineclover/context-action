...
function CatalogCommandGrid({
  ...
  commands: readonly MCPCommandReference[];
  commandFeedback: string | null;
  onCopy: (command: MCPCommandReference) => void;
  onDownload: (command: MCPCommandReference) => void;
}) {
  return (
      ...
      {commands.map((command) => (
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              <h3 className="font-semibold text-slate-950">{command.title}</h3>
          ...
          <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap justify-end gap-2">
                <button
                  className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-indigo-400"
                ...
                <button
                  className="rounded-full border border-slate-200 bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:border-indigo-400"
                  onClick={() => onDownload(command)}
                  type="button"
                >
          ...
          <div className="mt-4 grid gap-3 text-xs md:grid-cols-2">
            ...
            <div>
              <h4 className="font-semibold text-slate-800">예상 실행 체인</h4>
...
export default function McpFunctionCallingCatalog() {
  ...
  const copyCatalogCommand = (command: MCPCommandReference) => {
    void copyCommand(command.id, command.prompt);
  };

  const downloadCatalogCommand = (command: MCPCommandReference) => {
    ...
    <div className="min-h-screen bg-slate-50 px-6 py-10">
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-2">
          <article className="rounded-2xl border border-sky-200 bg-sky-50 p-6 shadow-sm md:p-8">
            <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.16em] text-sky-700">
                  Live Code Editor
                </p>
                <h2 className="mt-2 text-2xl font-semibold text-sky-950">
                  부모 소유 editor tool-chain
                </h2>
              </div>
              <Link
            ...
            <p className="mt-3 text-sm leading-6 text-sky-900">
              두 revision clock, iframe acknowledgement, filesystem save를
              포함한 browser-owned editor 계약입니다.
            </p>
            <CatalogCommandGrid
              commandFeedback={commandFeedback}
              commands={mcpLiveEditorCommands}
              onCopy={copyCatalogCommand}
              onDownload={downloadCatalogCommand}
            />
          </article>
          ...
          <article className="rounded-2xl border border-emerald-200 bg-emerald-50 p-6 shadow-sm md:p-8">
            ...
            <p className="mt-3 text-sm leading-6 text-emerald-900">
              단일 workspace revision과 semantic visual mutation, preview 결과를
              한 chain으로 확인합니다.
            </p>
            <CatalogCommandGrid
              commandFeedback={commandFeedback}
              commands={mcpRealtimeWebCodingCommands}
              onCopy={copyCatalogCommand}
              onDownload={downloadCatalogCommand}
            />
          </article>
