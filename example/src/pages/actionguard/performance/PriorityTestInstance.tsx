...
const PriorityGrid = memo(
  ({ configsWithDelay }: { configsWithDelay: HandlerConfig[] }) => {
    const priorityCountsStore = usePriorityTestStore('priorityCounts');
    const priorityCounts = useStoreValue(priorityCountsStore);
    ...
    return (
      <div className="flex flex-wrap gap-0.5">
        {Array.from({ length: 100 }, (_, index) => {
          ...
          const hasHandler = configsWithDelay.some(
          ...
          const getBackgroundStyle = () => {
            if (hasHandler) {
              ...
              if (count === 0)
                return { backgroundColor: '#dbeafe', borderColor: '#93c5fd' }; // 연한 파란색
              if (count <= 2)
                return { backgroundColor: '#bfdbfe', borderColor: '#60a5fa' }; // 중간 파란색
              if (count <= 5)
                return { backgroundColor: '#93c5fd', borderColor: '#3b82f6' }; // 진한 파란색
              return { backgroundColor: '#60a5fa', borderColor: '#2563eb' }; // 매우 진한 파란색
            } else if (count > 0) {
              // 점프로 실행된 경우: 실행 횟수에 따른 노란색 농도
              if (count <= 2)
                return { backgroundColor: '#fef3c7', borderColor: '#fbbf24' }; // 연한 노란색
              if (count <= 5)
                return { backgroundColor: '#fde68a', borderColor: '#f59e0b' }; // 중간 노란색
              if (count <= 10)
                return { backgroundColor: '#fbbf24', borderColor: '#d97706' }; // 진한 노란색
          ...
          return (
            ...
            >
              ...
              <div
                className="leading-none font-mono font-bold"
                style={{ fontSize: '8px' }}
              >
...
const PriorityTestInstance = memo(function PriorityTestInstance({
    ...
    configsWithDelay.forEach((config) => {
      actionRegister.register(
        ...
        async (payload, controller) => {
          ...
          // abortSignal 체크 (외부 AbortController 사용)
          ...
          const newAverageTime = Math.round(
            newExecutionTimes.reduce((sum, time) => sum + time, 0) /
          ...
          if (currentCount < 5 && config.jumpToPriority !== null) {
            console.log(
              `🔄 Priority Jump: P${config.priority} → P${config.jumpToPriority} (count: ${currentCount})`
            );
            controller.jumpToPriority(config.jumpToPriority);
          } else if (currentCount >= 5 && config.jumpToPriority !== null) {
            console.log(
              `⛔ Jump blocked: P${config.priority} count (${currentCount}) >= 5`
            );
          }
        },
        {
  ...
  const resetTest = useCallback(() => {
    ...
    console.log(
      '🔄 Manual reset completed - All stores and local state cleared'
    );
      ...
      <div className="mb-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
          ...
          <div className="p-2 bg-red-50 rounded text-center">
            <div className="text-xs text-red-700 whitespace-nowrap">
              최소~최대
            </div>
            ...
            return !testResults || testResults.length === 0 ? (
              ...
              <div>
                {testResults.slice(-2).map((result, index) => (
                  <div
                    key={index}
                    className="text-xs text-gray-700 mb-1 last:mb-0 truncate"
                  >
                    {result}
                  </div>
                ))}
