...
const ActionGuardContext =
  createActionContextPattern<ActionGuardTestActions>('ActionGuardTest');
...
function ActionGuardTest() {
  ...
  const updateMetrics = useCallback(
    (executionTime: number, type: 'throttled' | 'debounced' | 'normal') => {
      ...
      const avgTime =
        executionTimes.current.reduce((a, b) => a + b, 0) /
        executionTimes.current.length;

      setMetrics((prev) => ({
        totalDispatches: prev.totalDispatches + 1,
        throttledCount:
          type === 'throttled' ? prev.throttledCount + 1 : prev.throttledCount,
        debouncedCount:
          type === 'debounced' ? prev.debouncedCount + 1 : prev.debouncedCount,
  ...
  useEffect(() => {
    ...
    const unregister3 = actionRegister.register(
      'scrollEvent',
      ({ position }, controller) => {
        const actionStartTime = performance.now();
    ...
    // 버튼 클릭 액션
    const unregister4 = actionRegister.register(
      'buttonClick',
      ({ buttonId }, controller) => {
        const actionStartTime = performance.now();
    ...
    // 양식 검증 액션 (조건부 실행)
    const unregister5 = actionRegister.register(
      'validateForm',
      ({ formData }, controller) => {
        const actionStartTime = performance.now();
    ...
    // 비동기 작업 액션
    const unregister6 = actionRegister.register(
      'asyncOperation',
        ...
        try {
          ...
          actionLogger.logAction(
            ...
            { operationType, delay },
            {
              ...
              toast: {
                type: 'success',
                message: `${operationType} 완료 (${delay}ms)`,
              },
            }
          );
    ...
    // 에러 테스트 액션
    const unregister7 = actionRegister.register(
      'errorTest',
    ...
    // 우선순위 테스트 액션
    const unregister8 = actionRegister.register(
      'priorityTest',
  ...
  const handleAsyncTest = useCallback(
    (operationType: string, delay: number) => {
    ...
    const tests = [
      ...
      () =>
        dispatch('validateForm', {
          formData: { name: 'Test', email: 'test@example.com', age: '25' },
        }),
      () => dispatch('errorTest', { shouldFail: false }),
      () =>
        dispatch('priorityTest', {
          priority: 2,
          message: 'Bulk priority test',
        }),
    ];
  ...
  const togglePause = useCallback(() => {
    ...
    if (isPaused) {
      ...
      actionLogger.logSystem('테스트 일시정지', {
        context: 'ActionGuard Test',
      });
    }
    ...
    <div className="action-guard-test">
      <header className="page-header">
        ...
        <p className="page-description">
          dispatch 옵션(debounce, throttle, validation, condition 등)과 성능
          모니터링을 실시간으로 테스트하고 분석할 수 있습니다.
        </p>
      </header>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="metric-item">
            <div className="metric-value">
              {metrics.lastExecutionTime.toFixed(2)}ms
            </div>
      ...
      <div className="demo-card mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">
          복합적인 테스트 시나리오를 실행하여 ActionGuard의 성능과 안정성을
          검증합니다.
        </p>
