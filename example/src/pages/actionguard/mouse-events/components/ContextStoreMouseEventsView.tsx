 ...
 */
const StatusPanel = memo(({ mouseState }: { mouseState: MouseEventsStateData }) => {
 ...
 */
const StatisticsPanel = memo(({ mouseState }: { mouseState: MouseEventsStateData }) => {
 ...
 */
const PerformancePanel = memo(({ mouseState }: { mouseState: MouseEventsStateData }) => {
  // 렌더링 횟수 추적
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-green-50 rounded-lg">
      ...
      <div className="text-center">
        <div className="text-xl font-bold text-purple-600">{contextMetrics.computedValues}</div>
        <div className="text-xs text-gray-600">Computed Values</div>
      </div>
      <div className="text-center">
        <div className="text-xl font-bold text-orange-600">{contextMetrics.contextLayers}</div>
 ...
 */
const ContextStoreMouseEventsViewComponent = ({ mouseState, onReset }: ContextStoreMouseEventsViewProps) => {
      ...
      <DemoCard>
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-600">
            This demo showcases <strong>Context Store Pattern</strong> with createDeclarativeStores. 
            Uses a unified store with <strong>computed values integration</strong> and automatic 
            action handling for optimal performance and maintainability.
          </p>
        </div>
      ...
      <DemoCard variant="info">
        ...
        <div className="space-y-3 text-sm text-gray-700">
          <p>
            ...
            <br />
            This implementation uses createDeclarativeStores for a single, unified store 
            that manages all mouse state. Computed values are automatically updated and 
          ...
          <ul className="list-disc list-inside space-y-1 ml-2">
            <li><strong>Single Source of Truth:</strong> All mouse data in one store</li>
            <li><strong>Action Integration:</strong> Built-in action handling with context</li>
            <li><strong>Computed Values:</strong> Automatic lazy evaluation of derived data</li>
            <li><strong>Context Isolation:</strong> Provider-based scope management</li>
            <li><strong>Performance:</strong> Minimal re-renders with smart memoization</li>
          </ul>
          <p>
            <strong className="text-gray-900">vs Reactive Stores:</strong>
            <br />
            Context Store provides better integration with the @context-action framework, 
            simpler state management, and built-in action handling compared to multiple 
            separate stores.
          </p>
        </div>
      </DemoCard>
