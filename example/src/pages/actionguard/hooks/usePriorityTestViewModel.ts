import type { UnregisterFunction } from '@context-action/core';
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { usePriorityTestStore } from '../context/ActionTestContext';
import type { PriorityTestViewModel, ViewModelDependencies } from './types';

/**
 * ViewModel Layer: 상태 관리와 UI 바인딩
 * Model과 View 사이의 중재자 역할
 */
export function usePriorityTestViewModel(
  ...
  // 간소화된 상태 관리
  const unregisterFunctionsRef = useRef<Map<string, UnregisterFunction>>(
    new Map()
  );
  const [registeredHandlers, setRegisteredHandlers] = useState<Set<string>>(
    new Set()
  );
    ...
    try {
      ...
      configs.forEach((config) => {
        ...
        const unregister = actionRegister.register(
          'priorityTest',
          async (
            { testId, delay }: { testId: string; delay: number },
            controller: any
          ) => {
            // 우선순위 카운트 증가
            ...
            const currentCount = countManagement.getPriorityCount(
              config.priority
            );

            if (enableConsoleLog) {
              executionState.addTestResult(
                `[${timestamp}ms] 🟡 ${config.label} 시작 (지연: ${config.delay}ms, 파라미터: ${delay}ms, 핸들러ID: ${uniqueHandlerId}, 현재카운트: ${currentCount})`
              );
            }
            ...
            try {
              ...
              executionState.addHandlerExecutionTime(
                uniqueHandlerId,
                actualDelay
              );
              ...
              if (enableConsoleLog) {
                executionState.addTestResult(
                  `[${completionTimestamp}ms] 🟢 ${config.label} 완료 (실제 소요: ${actualDelay}ms)`
                );
              }
              ...
              if (
                ...
                const finalCount = countManagement.getPriorityCount(
                  config.priority
                );
                ...
                if (finalCount <= 4) {
                  if (enableConsoleLog) {
                    executionState.addTestResult(
                      `[${jumpTimestamp}ms] 🦘 ${config.label} → P${config.jumpToPriority} 점프 (실행횟수: ${finalCount}/4)`
                    );
                  }
                  ...
                  if (enableConsoleLog) {
                    executionState.addTestResult(
                      `[${jumpTimestamp}ms] 🚫 ${config.label} 점프 제한 (실행횟수: ${finalCount}/4 초과)`
                    );
                  }
                  controller.next();
                }
              ...
              if (enableConsoleLog) {
                executionState.addTestResult(
                  `[${errorTimestamp}ms] ❌ ${config.label} 실패: ${error}`
                );
              }
  ...
  const executeTest = useCallback(
    async (controllerContainer?: { controller?: AbortController }) => {
      if (executionState.isRunning) {
        if (enableConsoleLog) {
          console.log(
            '⚠️ [WARNING] Test already running, ignoring new execution request'
          );
        }
        return;
      }
      ...
      try {
        ...
        if (enableConsoleLog) {
          executionState.addTestResult(
            `[0ms] 🚀 우선순위 테스트 시작 (총 ${configs.length}개 핸들러)`
          );
        }
        ...
        // 완료 로그
        if (enableConsoleLog) {
          const dispatchCompleteTimestamp =
            Date.now() - executionState.startTime;
          executionState.addTestResult(
            `[${dispatchCompleteTimestamp}ms] 🏁 디스패치 완료`
          );
          executionState.addTestResult(
            `[${dispatchCompleteTimestamp}ms] ✅ 모든 핸들러 실행 완료`
          );
        }
      } catch (error) {
        const errorTimestamp = Date.now() - executionState.startTime;
        executionState.addTestResult(
          `[${errorTimestamp}ms] ❌ 테스트 실행 실패: ${error}`
        );
      } finally {
        executionState.completeTest();
      }
    },
    [enableConsoleLog, configs.length, dispatch, initializeTest]
  ); // 객체 의존성 제거
    ...
    configs.forEach((config) => {
      ...
      const unregister = actionRegister.register(
        ...
        async (
          ...
          const currentCount = countManagement.incrementPriorityCount(
            config.priority,
            config.id
          );
          ...
          if (enableConsoleLog) {
            executionState.addTestResult(
              `[${timestamp}ms] 🟡 ${config.label} 시작 (지연: ${config.delay}ms, 핸들러ID: ${uniqueHandlerId}, 현재카운트: ${currentCount})`
            );
          }
          ...
          if (config.priority === 300 && currentCount > 1) {
            ...
            executionState.triggerPipelineAbort(
              `${config.label}에서 조건부 중단 (카운트: ${currentCount})`
            );
            return;
          }
          ...
          if (config.label.includes('Lowest') && currentCount >= 4) {
            executionState.triggerPipelineAbort(
              `${config.label}에서 최대 실행 횟수 초과 중단`
            );
            return;
          }
          ...
          try {
            ...
            if (controller.aborted) {
              ...
              if (enableConsoleLog) {
                executionState.addTestResult(
                  `[${abortTimestamp}ms] ⛔ ${config.label} 중단됨 (abort 신호)`
                );
              }
              return;
            }
            ...
            if (controller.aborted) {
              ...
              if (enableConsoleLog) {
                executionState.addTestResult(
                  `[${abortTimestamp}ms] ⛔ ${config.label} 중단됨 (지연 후)`
                );
              }
              return;
            }
            ...
            if (
              ...
              if (finalCount <= 3) {
                if (enableConsoleLog) {
                  executionState.addTestResult(
                    `[${jumpTimestamp}ms] 🦘 ${config.label} → P${config.jumpToPriority} 점프 (카운트: ${finalCount})`
                  );
                }
                controller.jumpToPriority(config.jumpToPriority);
              } else {
                if (enableConsoleLog) {
                  executionState.addTestResult(
                    `[${jumpTimestamp}ms] 🚫 ${config.label} 점프 건너뜀 (카운트: ${finalCount} > 3)`
                  );
                }
                controller.next();
              }
            ...
            if (error instanceof Error && error.message.includes('Aborted')) {
              if (enableConsoleLog) {
                executionState.addTestResult(
                  `[${errorTimestamp}ms] ⛔ ${config.label} 중단됨`
                );
              }
