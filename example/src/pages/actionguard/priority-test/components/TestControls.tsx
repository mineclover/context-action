...
export const TestControls = memo<TestControlsProps>(({
  ...
  const handleDelayChange = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  const delayOptions = [
    ...
    { value: 50, label: '표준 딜레이 (50ms)', description: '실제 상황 시뮬레이션' },
  ] as const;

  return (
    <div className={`test-controls bg-white rounded-lg shadow-sm border p-4 ${className}`}>
      {/* 제어 버튼들 */}
      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 pt-2 border-t">
          <div className="flex items-center gap-4 text-sm text-gray-600">
            <span>
              📊 핸들러: <span className="font-medium text-gray-800">{configs.length}개</span>
            </span>
            
            <span>
              ⏱️ 딜레이: <span className="font-medium text-gray-800">{selectedDelay}ms</span>
            </span>
            
            <span>
              🔄 점프 핸들러: <span className="font-medium text-gray-800">
        ...
        <div className="text-xs text-gray-500 bg-gray-50 rounded-md p-2">
          💡 {delayOptions.find(opt => opt.value === selectedDelay)?.description}
        </div>
      </div>
