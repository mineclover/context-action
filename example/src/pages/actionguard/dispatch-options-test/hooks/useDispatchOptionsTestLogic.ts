...
export function useDispatchOptionsTestLogic() {
  ...
  const updateMetrics = useCallback(
    (executionTime: number, type: 'throttled' | 'debounced' | 'normal') => {
      ...
      const avgTime = newExecutionTimes.reduce((a, b) => a + b, 0) / newExecutionTimes.length;
  ...
  useEffect(() => {
    ...
    const unregisterScroll = register.register(
      ...
      ({ position }, controller) => {
        ...
        logAction('scrollEvent', { position }, {
          context: 'ActionGuard Test',
        });
    ...
    const unregisterButton = register.register(
      ...
      ({ buttonId }, controller) => {
        ...
        logAction('buttonClick', { buttonId }, {
          context: 'ActionGuard Test',
        ...
        addTestResult('Button Click', 'success', `버튼 ID: ${buttonId}`, execTime);
    ...
    const unregisterValidation = register.register(
      ...
      ({ formData }, controller) => {
        ...
        logAction('validateForm', { formData }, {
          context: 'ActionGuard Test',
          toast: { type: status === 'success' ? 'success' : 'error', message },
        });
    ...
    const unregisterAsync = register.register(
      ...
      async ({ operationType, delay }, controller) => {
        ...
        logAction('asyncOperation', { operationType, delay }, {
          context: 'ActionGuard Test',
          toast: { type: 'info', message: `비동기 작업 시작: ${operationType}` },
        });
        ...
        try {
          ...
          const execTime = performance.now() - actionStartTime;
          updateMetrics(execTime, 'normal');
          addTestResult('Async Operation', 'success', `작업 유형: ${operationType} (${delay}ms)`, execTime);
          
          logAction('asyncOperation', { result: 'completed' }, {
            context: 'ActionGuard Test',
            toast: { type: 'success', message: '비동기 작업 완료' },
          });
        } catch (error) {
          const execTime = performance.now() - actionStartTime;
          addTestResult('Async Operation', 'failed', `에러: ${error}`, execTime);
        }
    ...
    const unregisterError = register.register(
      ...
      ({ shouldFail }, controller) => {
        ...
        if (shouldFail) {
          logAction('errorTest', { shouldFail }, {
            context: 'ActionGuard Test',
            toast: { type: 'error', message: '의도적 에러 발생' },
          });
          
          const execTime = performance.now() - actionStartTime;
          addTestResult('Error Test', 'failed', '의도적으로 실패시킨 테스트', execTime);
    ...
    const unregisterConditional = register.register(
      ...
      ({ condition, data }, controller) => {
        ...
        if (condition) {
          logAction('conditionalAction', { condition, data }, {
            context: 'ActionGuard Test',
          ...
          const execTime = performance.now() - actionStartTime;
          updateMetrics(execTime, 'normal');
          addTestResult('Conditional Action', 'success', `조건 충족: ${data}`, execTime);
          controller.next();
        } else {
          logAction('conditionalAction', { condition, data }, {
            context: 'ActionGuard Test',
          ...
          const execTime = performance.now() - actionStartTime;
          addTestResult('Conditional Action', 'failed', '조건 불충족', execTime);
    ...
    const unregisterPriority = register.register(
      ...
      ({ priority, message }, controller) => {
        ...
        logAction('priorityTest', { priority, message }, {
          context: 'ActionGuard Test',
        ...
        const execTime = performance.now() - actionStartTime;
        updateMetrics(execTime, 'normal');
        addTestResult('Priority Test', 'success', `우선순위 ${priority}: ${message}`, execTime);
  ...
  const runBulkTest = useCallback(async () => {
    ...
    const tests = [
      ...
      () => dispatch('priorityTest', { priority: 100, message: 'bulk test priority' }),
    ];
  ...
  }, [dispatch, register, testState.bulkTestRunning, testState.formData, testStore]);
  ...
  return {
    ...
    validateForm: (formData: FormData) => dispatch('validateForm', { formData }),
    ...
    updateScrollPosition: (position: number) => dispatch('updateScrollPosition', { position }),
    updateFormData: (formData: Partial<FormData>) => dispatch('updateFormData', { formData }),
    
    // Control actions
    startAutoScrolling,
    stopAutoScrolling,
    togglePause: () => dispatch('togglePause', { isPaused: !testState.isPaused }),
    ...
    successRate: testState.testResults.length > 0 
      ? (testState.testResults.filter(r => r.status === 'success').length / testState.testResults.length * 100).toFixed(1)
      : '0',
  };
}