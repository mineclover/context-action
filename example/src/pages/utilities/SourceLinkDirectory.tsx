...
import { CodeBlock, CodeExample } from '../../components/ui/CodeExample';
import {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-green-700">
        버튼을 클릭하여 가상 컴포넌트를 마운트/언마운트하고 실시간으로 등록
        상태를 확인하세요.
      </p>
  ...
  return (
    <div className="bg-white border-2 border-green-300 rounded p-2 text-sm">
      <div className="font-medium text-green-800">
        🎭 Virtual Demo Component
      </div>
      <div className="text-green-600">Counter: {counter}</div>
      <div className="text-xs text-green-500">
        이 컴포넌트는 demo/VirtualDemo.tsx로 등록됩니다
      </div>
    </div>
  );
}
    ...
    <div className="space-y-3">
      ...
      <div className="bg-white border border-purple-200 rounded p-3 text-sm">
        <div className="font-medium text-purple-800">
          📋 Status for: {testFilePath}
        </div>
        <div className="mt-1 space-y-1">
          <div className="flex justify-between">
            <span>등록됨:</span>
            <span
              className={`font-medium ${isRegistered ? 'text-green-600' : 'text-red-600'}`}
            >
              {isRegistered ? '✅ Yes' : '❌ No'}
            </span>
          </div>
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-orange-700">
        동일한 컴포넌트의 여러 인스턴스를 생성하여 인스턴스 추적 기능을
        확인하세요.
      </p>
      <div className="flex gap-2">
        <button
      ...
      {instances.length > 0 && (
        <div className="text-xs text-orange-600 bg-orange-100 p-2 rounded">
          💡 위 파일 목록에서 "demo/MultiInstance.tsx" 항목의 숫자를
          확인해보세요!
        </div>
      )}
    </div>
    ...
    <div className="bg-white border-2 border-orange-300 rounded p-2 text-sm flex justify-between items-center">
      <div>
        <div className="font-medium text-orange-800">
          🔄 Multi Instance #{instanceId}
        </div>
        <div className="text-xs text-orange-500">
          demo/MultiInstance.tsx로 등록됨
        </div>
      </div>
      <button
        onClick={onRemove}
      ...
      <div className="space-y-6">
        ...
        <section className="prose prose-sm max-w-none">
          ...
          <p className="text-gray-600 mb-4">
            소스 링크 디렉터리는 예제 애플리케이션의 모든 페이지와 컴포넌트가
            자동으로 자신의 소스 코드 위치를 등록하고, GitHub에서 직접 확인할 수
            있는 링크를 제공하는 유틸리티 시스템입니다.
          </p>

          <h3 className="text-base font-medium mb-2">Key Features:</h3>
          <ul className="text-sm text-gray-600 space-y-1">
            <li>
              • <strong>Automatic Registration:</strong> 컴포넌트가 마운트될 때
              자동으로 소스 파일 등록
            </li>
            <li>
              • <strong>Instance Tracking:</strong> 동일한 컴포넌트의 여러
              인스턴스 추적
            </li>
            <li>
              • <strong>GitHub Integration:</strong> 소스 파일에 대한 직접
              GitHub 링크 제공
            </li>
            <li>
              • <strong>Category Organization:</strong> 파일 경로 기반 카테고리
              자동 분류
            </li>
            <li>
              • <strong>Real-time Updates:</strong> 페이지 네비게이션에 따른
              실시간 파일 목록 업데이트
            </li>
          </ul>
        </section>
    ...
    <div>
      ...
      <p>Active instances: {instanceCount}</p>
        ...
        <section>
          ...
          <p className="text-sm text-gray-600 mb-4">
            TypeScript 파일(`.ts`)은 React 훅을 직접 사용할 수 없으므로, Proxy
            Registration Pattern을 사용하여 등록합니다:
          </p>
  ...
  return <div>User Management</div>;
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-4">
            <h3 className="text-base font-medium mb-2 text-blue-800">
              📋 Proxy Registration 가이드라인
            </h3>
            <ul className="text-sm text-blue-700 space-y-2">
              <li>
                <strong>• 누가 등록하나?</strong> TypeScript 파일을 import하여
                사용하는 React 컴포넌트
              </li>
              <li>
                <strong>• 언제 등록하나?</strong> 컴포넌트 마운트 시점에 함께
                등록
              </li>
              <li>
                <strong>• 어떤 파일들?</strong> Business logic, Types, Utils,
                Constants 등 .ts 파일들
              </li>
              <li>
                <strong>• 태그 활용:</strong> 'typescript', 'pure-functions',
                'types' 등으로 분류
              </li>
              <li>
                <strong>• 우선순위:</strong> 30-40 범위 권장 (컴포넌트보다 낮게)
              </li>
            </ul>
          </div>
        </section>
        ...
        <section>
          <h2 className="text-lg font-semibold mb-3">
            실제 사용 예시: Layered Architecture
          </h2>
          <p className="text-sm text-gray-600 mb-4">
            <a
              href="/patterns/layered-architecture"
              className="text-blue-600 hover:underline"
            >
              Layered Architecture 패턴
            </a>
            에서 6-Layer 구조의 TypeScript 파일들이 어떻게 등록되는지 확인할 수
            있습니다:
          </p>
      ...
      <UserManagementStoreProvider>
        {/* ... */}
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 mt-4">
            <p className="text-sm text-green-700">
              <strong>✅ 결과:</strong> TypeScript 파일들이 Source Directory에
              자동으로 등록되어 GitHub 링크와 함께 6-Layer 구조별로 분류되어
              표시됩니다.
            </p>
          </div>
        </section>
  ...
  totalCount: number;
        ...
        <section>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <h3 className="text-base font-medium mb-2 text-green-800">
                실시간 등록 데모
              </h3>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              <h3 className="text-base font-medium mb-2 text-purple-800">
                파일 상태 체커
              </h3>
              <FileStatusChecker />
            </div>
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
              <h3 className="text-base font-medium mb-2 text-orange-800">
                인스턴스 카운터
              </h3>
              <InstanceCounterDemo />
            </div>
          ...
          <div className="bg-gray-50 border rounded p-4">
            ...
            {activeFiles.length === 0 ? (
              <div className="text-sm text-gray-500 py-4 text-center">
                No files registered. Navigate to other pages to see source
                files.
              </div>
            ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div className="bg-white border border-blue-200 rounded p-3">
              ...
              <div className="text-blue-600 text-xs">
                컴포넌트에서 소스 파일을 자동 등록하는 메인 훅
              </div>
            </div>
            <div className="bg-white border border-blue-200 rounded p-3">
              <div className="font-medium text-blue-800 mb-1">
                🗄️ SourceLinkRegistry
              </div>
              <div className="text-blue-600 text-xs">
                등록된 파일 정보를 관리하는 Store Context
              </div>
            </div>
            <div className="bg-white border border-blue-200 rounded p-3">
              <div className="font-medium text-blue-800 mb-1">
        ...
        <section className="prose prose-sm max-w-none">
          ...
          <div className="text-sm text-gray-600 space-y-2">
            <p>
              <strong>For Developers:</strong> 예제 코드를 보면서 실제 구현을
              바로 GitHub에서 확인
            </p>
            <p>
              <strong>For Learning:</strong> 각 페이지의 구현 방식을 쉽게
              탐색하고 학습
            </p>
            <p>
              <strong>For Documentation:</strong> 코드와 문서가 항상 동기화된
              상태 유지
            </p>
            <p>
              <strong>For Development:</strong> 컴포넌트 재사용성과 구조 파악이
              용이
            </p>
          </div>
        </section>
