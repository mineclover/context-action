...
last_update: '2025-08-30T10:57:35.529Z'
...
• **대규모 데이터 시각화** (차트, 대시보드)
• **메모리 제약 환경** (모바일, 임베디드)
• **폼 상태 관리** (사용자 입력, 유효성 검사)
• **UI 컴포넌트 상태** (모달, 드롭다운, 토글)
• **비즈니스 로직 상태** (사용자 프로필, 설정)
• **저빈도 업데이트** (알림, 상태 메시지)
• **디버깅 및 개발** (스토어 검사, 로깅)
• **관리자 인터페이스** (시스템 모니터링, 분석)
• **배치 작업** (데이터 내보내기, 대량 업데이트)
• **성능 프로파일링** (메트릭 수집, 벤치마킹)