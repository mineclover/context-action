...
last_update: '2025-08-26T00:34:27.361Z'
...
모든 예제는 표준화된 기본 액션 설정 명세를 사용합니다. 설정 기반 빠른 시작
...
모든 구현은 일관성과 유지보수성을 위해 설정 명세를 따릅니다. 주요 기능
...
- 스토어 패턴 - 상태 관리를 위한 스토어 설정과 결합
- 설정 패턴 - 모든 액션 구현의 기반
- 아키텍처 패턴 - MVVM 및 도메인 컨텍스트 통합
...
1. 설정 완료: 기본 액션 설정을 먼저 따르기
2. 기본부터 시작: 기본 사용법으로 시작하기
3. 고급 탐색: 준비되면 고급 패턴으로 이동
4. 확장: 복잡한 애플리케이션을 위해 등록 위임 사용.
