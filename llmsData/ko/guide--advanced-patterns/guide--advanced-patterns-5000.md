...
🔧 핸들러 상태 접근 패턴
MVVM 아키텍처 통합을 갖춘 도메인 컨텍스트에 걸친 고급 상태 접근 패턴. - 크로스 도메인 상태 접근: 모든 도메인에서 User, Product, UI 상태 접근
...
- 우선순위 성능 데모 - 크로스 도메인 우선순위 실행
- 검색 페이지 - 검증과 함께 멀티 도메인 검색
- 스크롤 페이지 - 성능 최적화된 UI 상태 관리

관련 패턴

설정 기반
- 멀티 컨텍스트 설정 - 완전한 아키텍처 기반
- MVVM 아키텍처 - 레이어 분리 패턴
- 도메인 컨텍스트 - 비즈니스 도메인 격리

기본 패턴
- 액션 기본 사용법 - 단일 컨텍스트 기본 패턴
- 타입 시스템 - 멀티 컨텍스트 TypeScript 통합
- 등록 위임 - 크로스 도메인 핸들러 구성.
