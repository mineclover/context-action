...
This inventory is intentionally separated from the canonical playbook examples. The current search finds 12 files. Each group needs either a Registry migration or an explicit compatibility wrapper before the structural check can be strict.
...
The next re-entry point is the remaining performance groups. Do not mark the migration complete until the 12-file inventory reaches zero or each remaining file is explicitly classified as a compatibility exception.
