...
Provider-order and canonical layer-path/name checking are now active alongside the direct-registration rule. The checker enforces Action → Store → Ref → Handler Registry nesting, recognizes 30 canonical roots, and currently reports zero provider-order and layer-path/name violations. `convention:check` is already part of `verify:all`, so CI catches new direct registrations and structural drift today.
...
- The convention checker now validates 30 canonical roots for layer paths, file names, and import boundaries; the conditional permission, action-based mouse, and context-store mouse usecases now expose their Context-Action boundaries under `contexts/`, packet/quote calculations remain in playbook Data Hooks, and the checker reports zero layer-path/name violations.
- `tools/context-action-lint/layered-surface-classification.json` now records both remaining non-canonical `handlers/` surfaces: one advanced comparison root and one compatibility object-context root; unclassified additions and stale classifications fail `convention:check`.
