[**context-action-monorepo v1.0.1**](../../../../README.md)

***

[context-action-monorepo](../../../../README.md) / [packages/tool-protocol/src](../README.md) / parseToolExecutionProvenance

# Function: parseToolExecutionProvenance()

> **parseToolExecutionProvenance**(`value`, `path?`): [`ToolExecutionProvenance`](../interfaces/ToolExecutionProvenance.md)

Defined in: packages/tool-protocol/src/execution-provenance.ts:74

Strictly validates a provenance record before it is persisted or exported.

## Parameters

### value

`unknown`

### path?

`string` = `'provenance'`

## Returns

[`ToolExecutionProvenance`](../interfaces/ToolExecutionProvenance.md)
