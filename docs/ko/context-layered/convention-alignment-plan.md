...
- `ChatUI`와 context-store 마우스 이벤트 컨테이너도 전용 Registry 모듈에서만 handler를 등록하도록 정리했습니다.
...
2. 직접 handler를 등록하는 도메인을 Handler Registry로 이동합니다. LogMonitor, ChatUI, context-store 마우스 이벤트는 완료했고, foundation/advanced 영역은 분류 또는 마이그레이션이 남아 있습니다.
