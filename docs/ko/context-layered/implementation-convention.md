...
현재 게이트는 canonical root 28곳을 검사하며 레이어 경로·명명 위반은
...
있는지 확인합니다. 현재 manifest에는 advanced 비교 표면 3곳과
compatibility object-context 표면 1곳이 있습니다. conditional permission
usecase는 canonical `contexts/`와 `handlers/` 표면에 편입되었고,
memoization 비교도 canonical `contexts/`와 `actions/` 경계로 이동했으며
기존 model과 hook 경로는 compatibility re-export로 유지합니다.
