...
Context-Action은 액션 디스패치와 스토어 관리를 위한 React 훅을 제공합니다. 이 가이드는 API 예제와 사용 패턴을 통해 **훅 사용 방법**을 다룹니다.
...
고급 시나리오를 위한 추가 훅들입니다.

### 스토어 관리

#### `useStoreManager()`
프로그래밍 방식으로 스토어를 업데이트하는 훅입니다.
...
function UserManager() {
  ...
  return (
    <div>
      <button onClick={() => updateUserName('새 이름')}>
        이름 업데이트
      </button>
...
function AdvancedUserComponent() {
  ...
  const handleAsyncAction = async () => {
    try {
      ...
      console.error('액션이 실패했습니다:', error);
    }
  };
...
1. **핸들러에 useCallback 사용** ([라이프사이클 가이드](/ko/guide/hooks-lifecycle) 참조):
\`\`\`typescript
useUserActionHandler('updateProfile', useCallback(async (payload) => {
  // 핸들러 로직
...
- 스토어 구독은 최소 리렌더링으로 최적화됨
- 핸들러 등록은 안정적인 참조를 사용
- 액션 디스패치는 자동으로 메모화됨

자세한 라이프사이클 이해를 위해 [훅 라이프사이클 가이드](/ko/guide/hooks-lifecycle)를 참조하세요.