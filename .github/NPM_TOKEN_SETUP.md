...
- 변경된 패키지가 없는 재실행도 빈 summary(`[]`)를 남기므로 consumer 검증과 artifact 업로드가
  동일한 계약으로 동작합니다.
- `@context-action/sem-doc`과 Foundation 패키지는
  `context-action-documentation-tooling`에서 publish하며 이 저장소의 workflow에서 배포하지 않습니다.
  tooling 저장소의 release workflow와 `verify:published-metadata`가 정본 release 검증을 담당합니다. 이 workflow는
  `@context-action/tool-protocol`과 `@context-action/tool-durable-operations`만 임시 consumer에
  설치해 public export를 `require`하는 smoke를 실행합니다. npm metadata 전파 지연은 polling으로
  흡수합니다.
