...
function deepClone<T>(target: T, options?: Options<any, any>): T;
function deepClone(target: any, options?: Options<any, any>) {
  if (!isDraftable(target, options)) return getValue(target);
  if (Array.isArray(target))
    return target.map((value) => deepClone(value, options));
  ...
  if (target instanceof Set) {
    const iterable = Array.from(target).map((value) =>
  ...
  const copy = Object.create(Object.getPrototypeOf(target));
  for (const key in target) copy[key] = deepClone(target[key], options);
  return copy;
}

export function cloneIfNeeded<T>(target: T, options?: Options<any, any>): T {
  return isDraft(target)
    ? (deepClone(getValue(target as object), options) as T)
    : target;
}
