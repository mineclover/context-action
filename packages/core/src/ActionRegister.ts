 ...
 * ```
 */
export type ActionPayloadMap = Record<string, any>;
...
export class ActionRegister<T extends ActionPayloadMap = ActionPayloadMap> {
	...
	register<K extends keyof T>(
		...
		if (pipeline.has(handlerId)) {
			this.logger.warn(
				`Handler with id ${handlerId} already exists for action: ${String(action)}`,
			);
			return () => {};
		}
		...
		return () => {
			...
			this.logger.debug(
				`Unregistered handler: ${handlerId} for action: ${String(action)}`,
			);
		};
	}
	 ...
	 * @returns Promise that resolves when all handlers complete
	 */
	async dispatch<K extends keyof T>(action: K, payload: T[K]): Promise<void>;
	/**
	...
	async dispatch<K extends keyof T>(action: K, payload?: T[K]): Promise<void> {
		...
		for (const { handler, config } of handlers) {
			...
			try {
				...
				this.logger.error(
					`Error in pipeline handler for action: ${String(action)}`,
					error,
				);
