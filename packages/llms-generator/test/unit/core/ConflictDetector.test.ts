...
describe('ConflictDetector', () => {
  ...
  describe('applyConflictResolutions()', () => {
    it('should apply exclude-conflicts strategy', () => {
      ...
      expect(resolution.resolvedDocuments.length).toBe(1);
      expect(resolution.excludedDocuments.length).toBe(1);
      expect(resolution.resolvedDocuments[0].document.id).toBe('doc-1'); // Higher priority
      expect(resolution.excludedDocuments[0].documentId).toBe('doc-2');
    });
    ...
    it('should apply higher-score-wins strategy', () => {
      ...
      expect(resolution.resolvedDocuments.length).toBe(1);
      expect(resolution.resolvedDocuments[0].document.id).toBe('high-score');
      expect(resolution.appliedResolutions.length).toBe(1);
      expect(resolution.appliedResolutions[0].action).toBe('exclude-second');
    });
    ...
    it('should apply keep-both strategy with warnings', () => {
      ...
      expect(resolution.warnings.length).toBe(1);
      expect(resolution.warnings[0].type).toBe('unresolved-conflict');
    });
    ...
    it('should handle manual-review conflicts', () => {
      ...
      expect(resolution.manualReviewRequired.length).toBe(1);
      expect(resolution.manualReviewRequired[0].conflictId).toBe('complex-conflict');
    });
  });
